...
use memscope_rs::core::targeted_optimizations::{FastStatsCollector, efficient_string_concat, BatchProcessor};
...
fn benchmark_unwrap_alternatives(c: &mut Criterion) {
    ...
    group.bench_function("unwrap_with_panic_check", |b| {
        let values: Vec<Option<i32>> = (0..1000).map(|i| if i % 10 == 0 { None } else { Some(i) }).collect();
        b.iter(|| {
            let mut sum = 0;
            for value in &values {
