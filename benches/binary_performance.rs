        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    ...
                    let result = binary::export_to_binary(
                        black_box(&allocations),
                        black_box(&binary_path)
                    );
...
fn bench_json_conversion(c: &mut Criterion) {
    ...
    for size in [100, 1000, 5000, 10000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("binary_to_json", size),
            size,
            |b, _| {
                b.iter(|| {
                    let result = binary::parse_binary_to_json(
                        black_box(&binary_path),
                        black_box(&json_path)
                    );
...
fn bench_file_sizes(c: &mut Criterion) {
    ...
    for size in [100, 1000, 5000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("size_comparison", size),
            size,
            |b, _| {
                b.iter(|| {
                    let temp_dir = TempDir::new().unwrap();
