...
fn demo_single_threaded() {
    ...
    println!("📊 Processed {} vectors with {} total elements", vectors.len(), total_elements);
...
fn demo_multi_threaded() {
    ...
    for thread_id in 0..num_threads {
        let handle = thread::spawn(move || {
            ...
            // Process data
            let sum: usize = thread_data.iter()
                .flat_map(|v| v.iter())
                .sum();
...
fn demo_async_simulation() {
    ...
    for task_id in 0..tasks {
        ...
        for step in 0..5 {
            ...
            // Process step data
            let _processed: Vec<_> = step_data.iter()
                .zip(task_data.iter())
                .map(|(a, b)| a + b)
                .collect();
...
fn demo_leak_simulation() {
    ...
    for cycle in 1..=20 {
        ...
        let current_items: usize = growing_collections.iter()
            .map(|collection| collection.len())
            .sum();
        
        println!("📈 Cycle {}: {} total items in memory", cycle, current_items);
