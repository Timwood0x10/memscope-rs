    ...
    if let Some(type_usage) = tracker.track_type_usage(type_name) {
        ...
        if !type_usage.usage_contexts.is_empty() {
            ...
            for context in &type_usage.usage_contexts {
                ...
                println!("       Avg execution time: {:.2} ns", context.performance_metrics.avg_execution_time_ns);
                println!("       Allocation frequency: {:.2}", context.performance_metrics.allocation_frequency);
                println!("       Cache miss rate: {:.1}%", context.performance_metrics.cache_miss_rate * 100.0);
            }
        }
        
        // Display usage timeline
        if !type_usage.usage_timeline.is_empty() {
            println!("   Usage timeline ({} data points):", type_usage.usage_timeline.len());
            for (i, point) in type_usage.usage_timeline.iter().take(3).enumerate() {
                println!("     {}. Time: {}, Usage: {}, Memory: {} bytes", 
                         i + 1, point.timestamp, point.usage_count, point.memory_usage);
            }
            if type_usage.usage_timeline.len() > 3 {
                println!("     ... and {} more data points", type_usage.usage_timeline.len() - 3);
            }
        }
        ...
        println!("     Memory efficiency: {:.1}/100", perf.memory_efficiency_score);
        println!("     CPU efficiency: {:.1}/100", perf.cpu_efficiency_score);
        println!("     Cache efficiency: {:.1}/100", perf.cache_efficiency_score);
        ...
        if !perf.optimization_recommendations.is_empty() {
            ...
            for (i, rec) in perf.optimization_recommendations.iter().enumerate() {
                println!("     {}. [{}] {:?}", i + 1, 
                         format_priority(&rec.priority), rec.recommendation_type);
