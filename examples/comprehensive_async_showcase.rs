...
use memscope_rs::async_memory::{
    AsyncResourceMonitor, TaskType, TaskId,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, config) in task_configs.iter().enumerate() {
        println!("  {}. {} [{:?}] - {:?} intensity", 
                 i + 1, config.name, config.task_type, config.intensity);
    }
    
    println!("\n🎯 Starting {} tasks with comprehensive monitoring...", task_configs.len());
    ...
    for (index, config) in task_configs.into_iter().enumerate() {
        ...
        // Start monitoring with detailed source location
        {
            ...
            let source_location = SourceLocation {
                ...
                function_name: format!("execute_{}_task", config.name.to_lowercase().replace(" ", "_")),
    ...
    for (task_id, task_name, handle) in task_handles {
        match tokio::time::timeout(Duration::from_secs(30), handle).await {
            Ok(Ok(result)) => {
                println!("✅ Task {} ({}) completed: {:?}", task_id, task_name, result);
                let mut mon = monitor.lock().unwrap();
...
fn create_comprehensive_task_configs() -> Vec<ComprehensiveTaskConfig> {
    vec![
        ComprehensiveTaskConfig {
            ...
            duration_secs: 15,
            description: "Large matrix operations with SIMD optimization".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            duration_secs: 20,
            description: "Real-time data processing with backpressure handling".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            intensity: IntensityLevel::Heavy,
            duration_secs: 12,
            description: "Recursive directory scanning with async file operations".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            intensity: IntensityLevel::Moderate,
            duration_secs: 18,
            description: "Concurrent HTTP requests with rate limiting".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            duration_secs: 10,
            description: "Large-scale in-memory data structures with LRU eviction".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            duration_secs: 25,
            description: "Low-priority cleanup and maintenance tasks".to_string(),
        },
        ComprehensiveTaskConfig {
            ...
            intensity: IntensityLevel::Heavy,
            duration_secs: 20,
            description: "Mixed CPU, IO, and network operations".to_string(),
        },
    ]
}
...
async fn execute_comprehensive_task(
    ...
    let update_handle = tokio::spawn(async move {
        for _ in 0..(config.duration_secs * 2) { // Update every 500ms
            {
                let mut mon = monitor_clone.lock().unwrap();
                mon.update_metrics(task_id);
            }
...
async fn execute_cpu_intensive_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    Ok(format!("CPU task completed with result checksum: {}", result))
}

async fn execute_io_intensive_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("💾 Starting IO intensive task: {} files of {}KB each", file_count, file_size / 1024);
    ...
    Ok(format!("IO task processed {} files, {} MB total", file_count, total_bytes / 1048576))
}

async fn execute_network_intensive_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("🌐 Starting network intensive task: {} simulated requests", request_count);
    ...
    for handle in handles {
        match handle.await {
            ...
            _ => {} // Ignore failures for this demo
        }
    }
    
    Ok(format!("Network task completed {} requests, {} KB transferred", 
               successful_requests, total_bytes / 1024))
}

async fn execute_memory_intensive_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("🧠 Starting memory intensive task: {} MB allocation", allocation_size / 1048576);
    ...
    for chunk in &allocations {
        for &byte in chunk.iter().step_by(1024) { // Sample every 1KB
            checksum = checksum.wrapping_add(byte as u64);
        }
        tokio::task::yield_now().await;
    }
    
    Ok(format!("Memory task allocated {} MB with checksum {}", 
               allocations.len(), checksum))
}

async fn execute_streaming_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    println!("📡 Starting streaming task: {} items/batch for {}s", batch_size, config.duration_secs);
    ...
    Ok(format!("Streaming task processed {} items in {} batches", 
               total_processed, batch_count))
}

async fn execute_background_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    println!("🔧 Starting background maintenance task for {}s", config.duration_secs);
    ...
    while std::time::Instant::now() < end_time {
        ...
        let _log_entry = format!("Maintenance cycle {} at {:?}", maintenance_cycles, std::time::Instant::now());
    ...
    Ok(format!("Background task completed {} maintenance cycles", maintenance_cycles))
}

async fn execute_mixed_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    Ok(format!("Mixed task: {} CPU ops, {} IO ops, {} network ops, {} memory ops", 
               cpu_work % 1000, io_files, network_requests, memory_allocations))
}

async fn execute_gpu_compute_task(config: &ComprehensiveTaskConfig) -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    ...
    Ok(format!("GPU compute task completed {} operations, result: {:.6}", 
               compute_iterations, compute_result))
}

async fn generate_json_report(monitor: &Arc<Mutex<AsyncResourceMonitor>>) -> Result<(), Box<dyn std::error::Error>> {
    let profiles = {
...
async fn generate_advanced_html_report(monitor: &Arc<Mutex<AsyncResourceMonitor>>) -> Result<(), Box<dyn std::error::Error>> {
    let profiles = {
        let mon = monitor.lock().unwrap();
        mon.get_all_profiles().clone()
    };
...
async fn print_performance_summary(monitor: &Arc<Mutex<AsyncResourceMonitor>>) -> Result<(), Box<dyn std::error::Error>> {
    let profiles = {
        let mon = monitor.lock().unwrap();
        mon.get_all_profiles().clone()
    };
    ...
    let total_duration: f64 = profiles.values()
        .filter_map(|p| p.duration_ms)
        .sum::<f64>() / 1000.0;
    
    let avg_efficiency = profiles.values()
        .map(|p| p.efficiency_score)
        .sum::<f64>() / total_tasks as f64;
    
    let total_memory_mb = profiles.values()
        .map(|p| p.memory_metrics.peak_bytes as f64 / 1_048_576.0)
        .sum::<f64>();
    
    let total_io_mb = profiles.values()
        .map(|p| (p.io_metrics.bytes_read + p.io_metrics.bytes_written) as f64 / 1_048_576.0)
        .sum::<f64>();
    
    let total_network_mb = profiles.values()
        .map(|p| (p.network_metrics.bytes_sent + p.network_metrics.bytes_received) as f64 / 1_048_576.0)
        .sum::<f64>();
    ...
    if let Some(best) = best_task {
        println!("\n🏆 Best Performing Task: {} ({:.1}% efficiency)", 
                 best.task_name, best.efficiency_score * 100.0);
    }
    
    if let Some(worst) = worst_task {
        println!("🔧 Needs Optimization: {} ({:.1}% efficiency)", 
                 worst.task_name, worst.efficiency_score * 100.0);
    }
    
    // Bottleneck analysis
    let bottleneck_counts = profiles.values()
    ...
    all_recommendations.sort_by(|a, b| 
        b.0.estimated_improvement.partial_cmp(&a.0.estimated_improvement).unwrap()
    );
    
    for (i, (rec, task_name)) in all_recommendations.iter().take(3).enumerate() {
        println!("  {}. {} ({}): {} ({:.1}% improvement)", 
                 i + 1, rec.category, task_name, rec.description, 
