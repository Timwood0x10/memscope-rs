...
impl EnhancedMemoryCoordinator {
    ...
    fn record_variable(
        ...
        self.variable_relationships
            .insert(var_name.clone(), (thread_id, task_id));
    }
    ...
    fn record_unified_variable(
        ...
        self.unified_variables
            .insert(unified_id.clone(), cross_module_data);

        // Record event chain
        let event = match state.as_str() {
            "Allocated" => Event::Allocation {
                var_id: unified_id.clone(),
                size,
            },
            "FFI_Shared" => Event::FFICrossing {
                var_id: unified_id.clone(),
                direction: "Rust_to_C".to_string(),
            },
            "Deallocated" => Event::Deallocation {
                var_id: unified_id.clone(),
            },
            _ => Event::Allocation {
                var_id: unified_id.clone(),
    ...
    fn analyze_lens_correlations(&mut self) {
        ...
        if !memory_hotspots.is_empty() {
            let correlation = CrossLensCorrelation {
                ...
                correlation_data: format!(
                    "Memory hotspots detected in threads: {:?}",
                    memory_hotspots
                ),
        ...
        if !ffi_threads.is_empty() {
            let correlation = CrossLensCorrelation {
                ...
                correlation_data: format!(
                    "FFI interactions detected in threads: {:?}",
                    ffi_threads
                ),
        ...
        println!(
            "  🔄 Lens correlations analyzed: {} active",
            self.lens_context.active_correlations.len()
        );
    }
...
async fn run_coordinated_workload(
        ...
        let handle = std::thread::spawn(move || {
            ...
            for var_idx in 0..48 {
                ...
                let traced_data = if var_idx % 5 == 0 {
                    ...
                    println!(
                        "    🔒 Thread {} owned-tracked: {} (lifecycle managed)",
                        thread_id, var_name
                    );
                    Some(tracked)
                } else {
                ...
                if has_ffi {
                    ...
                    println!(
                        "    🛡️ Thread {} tracked FFI: {} (boundary crossing)",
                        thread_id, var_name
                    );
                ...
                let final_state = if var_idx % 3 == 0 {
                    ...
                    if let Some(owned) = traced_data {
                        drop(owned);
                        println!(
                            "    🔒 Thread {} auto-deallocated owned tracking",
                            thread_id
                        );
                    }
                    "Deallocated"
            ...
            println!(
                "  ✅ Thread {} completed with real memory operations",
                thread_id
            );
        });
        handles.push(handle);
...
async fn generate_real_hybrid_data(
    ...
    memscope_rs::export::fixed_hybrid_template::HybridAnalysisData,
    Box<dyn std::error::Error>,
> {
    let coord = coordinator.lock().unwrap();

    println!(
        "  📊 Processing {} threads, {} tasks, {} variables",
    ...
    if let Some(hottest_thread) = coord
        .thread_memories
        .iter()
        .max_by_key(|t| t.allocated_bytes)
    {
        println!(
            "  🔥 Memory hotspot: Thread {} with {} bytes",
            hottest_thread.thread_id, hottest_thread.allocated_bytes
        );
    }
    ...
    let lifecycle_counts: std::collections::HashMap<String, usize> = coord
        .global_variables
        .iter()
...
fn print_detailed_relationships(
...
fn print_cross_thread_analysis(
    data: &memscope_rs::export::fixed_hybrid_template::HybridAnalysisData,
) {
    ...
    for (thread_id, &memory) in thread_memory_usage.iter().enumerate() {
        ...
        println!(
            ...
            memory / 1024,
            percentage,
            thread_var_counts[thread_id]
        );
    }

    // Identify primary memory consumption hotspot
    let max_memory_thread = thread_memory_usage
        .iter()
        .enumerate()
    ...
    println!(
        "🔥 Memory Hotspot: Thread {} ({}KB)",
        max_memory_thread,
        thread_memory_usage[max_memory_thread] / 1024
    );
}

/// Print task distribution analysis  
fn print_task_distribution_analysis(
    data: &memscope_rs::export::fixed_hybrid_template::HybridAnalysisData,
) {
...
fn print_variable_lifecycle_flow(
    data: &memscope_rs::export::fixed_hybrid_template::HybridAnalysisData,
) {
    ...
    println!(
        "Average Memory per Variable: {:.1} KB",
        if total_variables > 0 {
            total_memory as f64 / total_variables as f64 / 1024.0
        } else {
            0.0
        }
    );
    println!(
        "Memory Distribution Efficiency: {:.1}%",
...
fn calculate_memory_distribution_efficiency(
    ...
    let variance: f64 = thread_memories
        .iter()
...
fn print_analysis_summary(data: &memscope_rs::export::fixed_hybrid_template::HybridAnalysisData) {
    ...
    for variable in data.variable_registry.values() {
        *lifecycle_counts
            .entry(format!("{:?}", variable.lifecycle_stage))
            .or_insert(0) += 1;
    }
    ...
    for (idx, var) in sorted_vars.iter().take(5).enumerate() {
        println!(
            ...
            idx + 1,
            var.name,
            var.memory_usage as f64 / 1024.0,
            var.thread_id,
            var.lifecycle_stage
        );
    }
