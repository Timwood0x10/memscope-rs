...
fn main() {
    ...
    println!("✅ Created original Rc<String> (ref_count: {})", Rc::strong_count(&original_data));
    
    // Create clones and track relationships
    let clone1 = original_data.clone();
    let _tracked_clone1 = track_var!(clone1);
    println!("✅ Created clone1 (ref_count: {})", Rc::strong_count(&original_data));
    
    let clone2 = original_data.clone();
    let _tracked_clone2 = track_var!(clone2);
    println!("✅ Created clone2 (ref_count: {})", Rc::strong_count(&original_data));
    
    let clone3 = original_data.clone();
    let _tracked_clone3 = track_var!(clone3);
    println!("✅ Created clone3 (ref_count: {})", Rc::strong_count(&original_data));
    
    // Create weak references
    let weak1: RcWeak<String> = Rc::downgrade(&original_data);
    let _tracked_weak1 = track_var!(weak1);
    println!("✅ Created weak1 (weak_count: {})", Rc::weak_count(&original_data));
    
    let weak2: RcWeak<String> = Rc::downgrade(&original_data);
    let _tracked_weak2 = track_var!(weak2);
    println!("✅ Created weak2 (weak_count: {})", Rc::weak_count(&original_data));
    ...
    println!("✅ Created original Arc<Vec<i32>> (ref_count: {})", Arc::strong_count(&shared_vec));
    
    let arc_clone1 = shared_vec.clone();
    let _tracked_arc_clone1 = track_var!(arc_clone1);
    println!("✅ Created Arc clone1 (ref_count: {})", Arc::strong_count(&shared_vec));
    
    let arc_clone2 = shared_vec.clone();
    let _tracked_arc_clone2 = track_var!(arc_clone2);
    println!("✅ Created Arc clone2 (ref_count: {})", Arc::strong_count(&shared_vec));
    
    let arc_weak: ArcWeak<Vec<i32>> = Arc::downgrade(&shared_vec);
    let _tracked_arc_weak = track_var!(arc_weak);
    println!("✅ Created Arc weak reference (weak_count: {})", Arc::weak_count(&shared_vec));
    ...
    // Add some nested data
    {
        ...
        let _tracked_vec1 = track_var!(vec_data1.clone());
        let _tracked_vec2 = track_var!(vec_data2.clone());
        
        // Create clones of the Arc data
        let vec_clone1 = vec_data1.clone();
        let vec_clone2 = vec_data2.clone();
    ...
    let _tracked_shared = track_var!(shared_string.clone());
    println!("Initial ref_count: {}", Rc::strong_count(&shared_string));
    
    {
        let temp_clone1 = shared_string.clone();
        let _tracked_temp1 = track_var!(temp_clone1);
        println!("After temp_clone1: {}", Rc::strong_count(&shared_string));
        
        {
            let temp_clone2 = shared_string.clone();
            let _tracked_temp2 = track_var!(temp_clone2);
            println!("After temp_clone2: {}", Rc::strong_count(&shared_string));
            
            {
                let temp_clone3 = shared_string.clone();
                let _tracked_temp3 = track_var!(temp_clone3);
                println!("After temp_clone3: {}", Rc::strong_count(&shared_string));
            }
            println!("After temp_clone3 dropped: {}", Rc::strong_count(&shared_string));
        }
        println!("After temp_clone2 dropped: {}", Rc::strong_count(&shared_string));
    }
    println!("After temp_clone1 dropped: {}", Rc::strong_count(&shared_string));
    
    // Test data lifetime vs instance lifetime
    println!("\n🔍 Testing data lifetime vs instance lifetime:");
    
    let data_owner = Rc::new(42i32);
    let _tracked_owner = track_var!(data_owner.clone());
    let data_ptr = Rc::as_ptr(&data_owner) as usize;
    println!("Created data owner, data_ptr: 0x{:x}", data_ptr);
    
    let instance1 = data_owner.clone();
    let _tracked_instance1 = track_var!(instance1.clone());
    println!("Created instance1, same data_ptr: 0x{:x}", Rc::as_ptr(&instance1) as usize);
    
    let instance2 = data_owner.clone();
    let _tracked_instance2 = track_var!(instance2.clone());
    println!("Created instance2, same data_ptr: 0x{:x}", Rc::as_ptr(&instance2) as usize);
    
    // Drop the original owner
    drop(_tracked_owner);
    println!("Dropped original owner, ref_count: {}", Rc::strong_count(&instance1));
    ...
    let _tracked_weak = track_var!(weak_ref.clone());
    println!("Created weak reference, can upgrade: {}", weak_ref.upgrade().is_some());
    
    // Try multiple upgrades
    for i in 0..3 {
        if let Some(upgraded) = weak_ref.upgrade() {
            let _tracked_upgraded = track_var!(upgraded);
            println!("Upgrade {} successful, ref_count: {}", i + 1, Rc::strong_count(&strong_ref));
        }
    }
