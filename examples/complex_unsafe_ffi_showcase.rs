...
use memscope_rs::{export_interactive_html, get_global_tracker, init};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        ...
        let mut unsafe_union = UnsafeUnion {
            as_u64: 0x123456789ABCDEF0,
        };
    ...
    for allocation in &enhanced_allocations {
        let source_name = match &allocation.source {
            ...
            memscope_rs::unsafe_ffi_tracker::AllocationSource::FfiC { library_name, .. } => {
                *library_counts.entry(library_name).or_insert(0) += 1;
                "FFI"
            }
            memscope_rs::unsafe_ffi_tracker::AllocationSource::CrossBoundary { .. } => {
                "Cross-boundary"
            }
        };
    ...
    println!(
        "   ✅ Moderate complexity unsafe/FFI dashboard exported (using standard memory analysis)"
    );
