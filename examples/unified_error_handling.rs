...
use memscope_rs::core::{
    ...
    MemScopeError, MemoryOperation, SystemErrorType, TrackingError, TrackingResult,
};
...
fn demonstrate_new_error_creation() {
    ...
    let alloc_error =
        MemScopeError::memory(MemoryOperation::Allocation, "Failed to allocate 1024 bytes");
    println!("   Memory error: {}", alloc_error);
    println!(
        "   Category: {}, Severity: {:?}, Recoverable: {}",
        alloc_error.category(),
        alloc_error.severity(),
        alloc_error.is_recoverable()
    );

    // Analysis errors
    let analysis_error =
        MemScopeError::analysis("borrow_checker", "Mutable borrow conflict detected");
...
fn demonstrate_error_recovery() {
    ...
    let recoverable_error =
        MemScopeError::memory(MemoryOperation::Allocation, "Temporary allocation failure");
    println!("   Error: {}", recoverable_error);
    println!(
        "   Can recover: {}",
        recovery.can_recover(&recoverable_error)
    );
...
fn simulate_old_function() -> TrackingResult<i32> {
    ...
    if std::env::var("SIMULATE_SUCCESS").is_ok() {
        ...
        Err(TrackingError::AllocationFailed(
