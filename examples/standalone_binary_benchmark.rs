...
impl BenchmarkResult {
    ...
    fn compare_with(&self, baseline: &BenchmarkResult) {
        ...
        let serialize_speedup = baseline.serialize_time.as_nanos() as f64 / self.serialize_time.as_nanos() as f64;
        let deserialize_speedup = baseline.deserialize_time.as_nanos() as f64 / self.deserialize_time.as_nanos() as f64;
        let size_reduction = baseline.file_size as f64 / self.file_size as f64;
        
        println!("\n🎯 {} vs {} Improvement:", self.format_name, baseline.format_name);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Generated {} allocations with full analysis data", test_data.allocations.len());
...
fn create_realistic_memory_data() -> MemoryAnalysisExport {
    ...
    for (type_name, count, size_range) in &type_patterns {
        ...
        for i in 0..*count {
            ...
            let lifetime = if i % 10 == 0 { None } else { Some((i as u64 % 5000) + 100) }; // Some leaks
            ...
            allocations.push(MemoryAllocation {
                ...
                timestamp_dealloc: if is_leaked { None } else { Some(base_time + i as u64 + lifetime.unwrap()) },
        ...
        let avg_lifetime = if *count > 0 { total_lifetime / *count as f64 } else { 0.0 };
        type_usage_stats.insert(type_name.to_string(), TypeUsageInfo {
    ...
    let memory_leaks: Vec<LeakInfo> = allocations.iter()
...
fn benchmark_json(data: &MemoryAnalysisExport) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    // Serialize
...
fn benchmark_messagepack(data: &MemoryAnalysisExport) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
    // Serialize
    let serialize_start = Instant::now();
...
fn benchmark_compressed(data: &MemoryAnalysisExport) -> Result<BenchmarkResult, Box<dyn std::error::Error>> {
...
fn print_comprehensive_analysis(json: &BenchmarkResult, msgpack: &BenchmarkResult, compressed: &BenchmarkResult) {
