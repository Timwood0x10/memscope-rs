//! Memory stress test example showing trace_tools under extreme load
...
fn main() {
    println!("🔥 Memory Stress Test - Pushing trace_tools to the limit!");
...
fn massive_allocation_burst() {
    ...
    println!("  Rate: {:.0} allocations/second", target_count as f64 / duration.as_secs_f64());
...
fn concurrent_allocation_storm() {
    ...
    println!("  {} threads allocated {} objects total", num_threads, total_allocated);
}
...
fn large_object_stress_test() {
    ...
    for i in 0..100 {
        ...
        if i % 10 == 0 {
            println!("  Allocated large object {} ({:.2} MB)", i, size as f64 / 1024.0 / 1024.0);
        }
    }
    
    // Calculate total size
    let total_size: usize = large_objects.iter().map(|(data, _)| data.len()).sum();
    println!("  Total large object size: {:.2} MB", total_size as f64 / 1024.0 / 1024.0);
    
    // Drop half of them
    large_objects.truncate(50);
    let remaining_size: usize = large_objects.iter().map(|(data, _)| data.len()).sum();
    println!("  Remaining after cleanup: {:.2} MB", remaining_size as f64 / 1024.0 / 1024.0);
}
...
fn rapid_alloc_dealloc_cycles() {
    ...
    println!("  Completed {} rapid cycles with {} objects each", cycles, objects_per_cycle);
}

/// Print comprehensive stress test results
fn print_final_analysis(tracker: &trace_tools::MemoryTracker, duration: Duration) {
    ...
    if let Ok(stats) = tracker.get_stats() {
        ...
        println!("  🎯 Peak memory usage: {:.2} MB", stats.peak_memory as f64 / 1024.0 / 1024.0);
        println!("  🎯 Current active memory: {:.2} MB", stats.active_memory as f64 / 1024.0 / 1024.0);
    ...
    if let Ok(breakdown) = tracker.get_memory_by_type() {
        ...
        for (i, usage) in sorted_breakdown.iter().take(10).enumerate() {
            println!("  {}. {}: {:.2} KB", i + 1, usage.type_name, usage.total_size as f64 / 1024.0);
        }
    }
    ...
    println!("\n💪 trace_tools successfully handled extreme memory stress!");
