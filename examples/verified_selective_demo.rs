...
use memscope_rs::lockfree::{
    PlatformResourceCollector, IntegratedProfilingSession,
    export_comprehensive_analysis, generate_comprehensive_html_report,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
            ...
            while active.load(Ordering::Relaxed) > 0 {
                if let Ok(metric) = collector.collect_metrics() {
                    ...
                    if sample_count % 10 == 0 {
                        println!("   📊 Resource sample {}: CPU {:.1}%, Threads {}", 
                                sample_count, 
    ...
    let handles: Vec<_> = (0..50)
        .map(|thread_idx| {
            ...
            thread::spawn(move || -> Result<(), String> {
                run_enhanced_verified_worker(thread_idx, &output_dir, &total_operations, &tracking_log)
            })
        })
        .collect();
            ...
            Ok(analysis) => {
                ...
                match export_comprehensive_analysis(&analysis, &output_dir, "platform_demo") {
                    Ok(()) => {
                        ...
                        println!("   📈 Check ./Memoryanalysis/platform_demo_resource_rankings.json");
                    }
                    Err(e) => {
    ...
    if !resource_summary.is_empty() {
        ...
        let cpu_usages: Vec<f32> = resource_summary.iter()
        ...
        println!("   🔥 CPU usage - Avg: {:.1}%, Peak: {:.1}%", avg_cpu, max_cpu);
        println!("   🧵 CPU cores detected: {}", 
                resource_summary.first().map(|(_, m)| m.cpu_metrics.per_core_usage.len()).unwrap_or(0));
    ...
    if let Some(_analysis) = comprehensive_analysis {
        ...
        if json_file.exists() {
            ...
            println!("   ✅ JSON export: {} ({} bytes)", json_file.display(), file_size);
        } else {
        ...
        if html_file.exists() {
            ...
            println!("   ✅ HTML dashboard: {} ({} bytes)", html_file.display(), file_size);
        } else {
        ...
        if rankings_file.exists() {
            ...
            println!("   ✅ Resource rankings: {} ({} bytes)", rankings_file.display(), file_size);
        } else {
