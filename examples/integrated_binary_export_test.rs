...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  📏 File size: {} bytes ({:.2} MB)", json_size, json_size as f64 / 1024.0 / 1024.0);
    ...
    println!("  📏 File size: {} bytes ({:.2} MB)", fast_size, fast_size as f64 / 1024.0 / 1024.0);
    println!("  🗜️  Compression: {:.1}%", fast_stats.compression_ratio * 100.0);
    
    // Compact configuration
    println!("\n💾 Compact Configuration:");
    let compact_start = Instant::now();
    let compact_options = BinaryExportOptions::compact();
    let compact_stats = tracker.export_to_binary("integrated_test_compact.msgpack", compact_options)?;
    ...
    println!("  📏 File size: {} bytes ({:.2} MB)", compact_size, compact_size as f64 / 1024.0 / 1024.0);
    println!("  🗜️  Compression: {:.1}%", compact_stats.compression_ratio * 100.0);
    
    // Selective configuration
    println!("\n🎯 Selective Configuration:");
    let selective_start = Instant::now();
    let selective_options = BinaryExportOptions::selective();
    let selective_stats = tracker.export_to_binary("integrated_test_selective.msgpack", selective_options)?;
    ...
    println!("  📏 File size: {} bytes ({:.2} MB)", selective_size, selective_size as f64 / 1024.0 / 1024.0);
    println!("  🗜️  Compression: {:.1}%", selective_stats.compression_ratio * 100.0);
    ...
    let full_data = tracker.load_from_binary("integrated_test_selective.msgpack")?;
    let load_all_duration = load_all_start.elapsed();
    println!("  📊 Full load: {} allocations in {:?}", full_data.allocations.len(), load_all_duration);
    ...
    let selective_data = tracker.load_selective_binary("integrated_test_selective.msgpack", criteria)?;
    let load_selective_duration = load_selective_start.elapsed();
    println!("  🎯 Selective load: {} Vec<i32> allocations in {:?}", selective_data.len(), load_selective_duration);

    // Print comprehensive comparison
    print_comprehensive_comparison(
        json_duration, json_size,
        fast_duration, fast_size, &fast_stats,
        compact_duration, compact_size, &compact_stats,
        selective_duration, selective_size, &selective_stats,
    );
...
fn create_comprehensive_test_data() {
    ...
    for i in 0..300 {
        ...
        track_var!(vec_data, format!("performance_vector_{}", i));
        
        let map_data: HashMap<String, i32> = (0..50)
            .map(|j| (format!("key_{}_{}", i, j), j))
            .collect();
        track_var!(map_data, format!("performance_hashmap_{}", i));
    }
    ...
    for i in 0..150 {
        let nested: Vec<Vec<String>> = (0..10)
            ...
            .collect();
        track_var!(nested, format!("nested_structure_{}", i));
    }
    
    // Create temporary objects with varying lifetimes
    for i in 0..500 {
        let temp_string = format!("Temporary string with substantial content for testing compression efficiency {}", i);
        track_var!(temp_string, format!("temp_string_{}", i));
    }
    
    // Create long-lived objects
    for i in 0..100 {
        let long_lived: Box<Vec<u64>> = Box::new((0..200).map(|x| x as u64).collect());
        track_var!(long_lived, format!("long_lived_box_{}", i));
    }
    ...
    for i in 0..100 {
        ...
        track_var!(rc_data, format!("rc_data_{}", i));
        
        let arc_data = Arc::new(vec![i; 100]);
        track_var!(arc_data, format!("arc_data_{}", i));
    }
...
fn print_comprehensive_comparison(
    ...
    println!("  Compact Binary:   {:.1}x faster than JSON", compact_speedup);
    println!("  Selective Binary: {:.1}x faster than JSON", selective_speedup);
    ...
    println!("  Fast Binary:      {:.1}x smaller than JSON", fast_size_reduction);
    println!("  Compact Binary:   {:.1}x smaller than JSON", compact_size_reduction);
    println!("  Selective Binary: {:.1}x smaller than JSON", selective_size_reduction);
    
    // Performance summary table
    println!("\n📊 Performance Summary Table:");
    println!("  Format           | Export Time | File Size  | Speedup | Size Reduction");
    println!("  -----------------|-------------|------------|---------|---------------");
    println!("  JSON (baseline)  | {:>9.1}ms | {:>8.1}KB |   1.0x  |        1.0x", 
             json_duration.as_secs_f64() * 1000.0,
             json_size as f64 / 1024.0);
    println!("  Fast Binary      | {:>9.1}ms | {:>8.1}KB | {:>6.1}x  | {:>10.1}x", 
             fast_duration.as_secs_f64() * 1000.0,
             fast_size as f64 / 1024.0,
             fast_speedup,
             fast_size_reduction);
    println!("  Compact Binary   | {:>9.1}ms | {:>8.1}KB | {:>6.1}x  | {:>10.1}x", 
             compact_duration.as_secs_f64() * 1000.0,
             compact_size as f64 / 1024.0,
             compact_speedup,
             compact_size_reduction);
    println!("  Selective Binary | {:>9.1}ms | {:>8.1}KB | {:>6.1}x  | {:>10.1}x", 
    ...
    println!("\n🔍 Key Insights:");
    println!("  • Binary formats show consistent {:.1}x-{:.1}x speed improvements", 
             [compact_speedup, fast_speedup, selective_speedup].iter().fold(f64::INFINITY, |a, &b| a.min(b)),
             [compact_speedup, fast_speedup, selective_speedup].iter().fold(0.0, |a, &b| a.max(b)));
    println!("  • File size reductions range from {:.1}x to {:.1}x smaller", 
             [fast_size_reduction, compact_size_reduction, selective_size_reduction].iter().fold(f64::INFINITY, |a, &b| a.min(b)),
             [fast_size_reduction, compact_size_reduction, selective_size_reduction].iter().fold(0.0, |a, &b| a.max(b)));
    println!("  • Compression ratios: Fast {:.1}%, Compact {:.1}%, Selective {:.1}%",
