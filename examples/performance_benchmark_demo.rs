...
fn benchmark_basic_operations() -> TrackingResult<()> {
    ...
    tracing::info!("  Allocation tracking: {:.2} ops/ms", operation_count as f64 / alloc_duration.as_millis() as f64);
    tracing::info!("  Variable association: {:.2} ops/ms", operation_count as f64 / assoc_duration.as_millis() as f64);
    tracing::info!("  Deallocation tracking: {:.2} ops/ms", operation_count as f64 / dealloc_duration.as_millis() as f64);
    tracing::info!("  Statistics retrieval: {:.2} ops/ms", 1000.0 / stats_duration.as_millis() as f64);
...
fn benchmark_allocation_patterns() -> TrackingResult<()> {
    ...
    tracing::info!("  Sequential pattern: {:.2}ms", sequential_duration.as_millis());
    tracing::info!("  Random-size pattern: {:.2}ms", random_size_duration.as_millis());
    tracing::info!("  Mixed alloc/dealloc pattern: {:.2}ms", mixed_duration.as_millis());
    ...
    tracing::info!("  Sequential: {} active, {}KB memory", stats1.active_allocations, stats1.active_memory / 1024);
    tracing::info!("  Random-size: {} active, {}KB memory", stats2.active_allocations, stats2.active_memory / 1024);
    tracing::info!("  Mixed: {} active, {}KB memory", stats3.active_allocations, stats3.active_memory / 1024);
...
fn benchmark_export_performance() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for &size in &test_sizes {
        ...
        for i in 0..size {
            ...
            tracker.associate_var(ptr, format!("export_var_{i}"), 
                match i % 4 {
                    0 => "i32".to_string(),
                    1 => "String".to_string(),
                    2 => "Vec<u8>".to_string(),
        ...
        tracing::info!("  Binary export: {:.2}ms ({}KB)", binary_export_duration.as_millis(), binary_size / 1024);
        tracing::info!("  JSON export: {:.2}ms ({}KB)", json_export_duration.as_millis(), json_size / 1024);
        tracing::info!("  HTML export: {:.2}ms ({}KB)", html_export_duration.as_millis(), html_size / 1024);
        tracing::info!("  Throughput: {:.2} allocs/ms (JSON)", size as f64 / json_export_duration.as_millis() as f64);
    }
...
fn benchmark_concurrent_operations() -> TrackingResult<()> {
    ...
    let handles: Vec<_> = (0..thread_count).map(|thread_id| {
        ...
        std::thread::spawn(move || -> TrackingResult<()> {
            for i in 0..operations_per_thread {
                ...
                tracker_clone.associate_var(ptr, 
    ...
    tracing::info!("  Concurrent: {} total, {} active", 
        concurrent_stats.total_allocations, concurrent_stats.active_allocations);
    tracing::info!("  Sequential: {} total, {} active", 
        sequential_stats.total_allocations, sequential_stats.active_allocations);
...
fn analyze_memory_efficiency() -> TrackingResult<()> {
    ...
    for &size in &allocation_sizes {
        ...
        tracing::info!("Size {}B: efficiency {:.1}% (expected {}B, actual {}B)", 
            size, efficiency, expected_memory, memory_increase);
    }
