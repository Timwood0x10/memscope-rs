...
use memscope_rs::export::binary::{
    export_to_binary_with_config, AdvancedMetricsLevel, BinaryExportConfig,
...
fn demo_performance_first_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        config.fragmentation_analysis
    );
    println!(
        "   Estimated performance impact: {:.1}%",
...
fn demo_custom_config_builder() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        ...
        config.thread_context_tracking
    );
