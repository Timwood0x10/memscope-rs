...
use memscope_rs::export::simd_optimizations::{SimdProcessor, SimdCapability};
...
fn test_u64_array_operations(processor: &SimdProcessor) {
    ...
    match (simd_encode_result, scalar_encode_result) {
        (Ok(simd_len), Ok(scalar_len)) => {
            println!("Encoded {} values ({} bytes each)", test_values.len(), simd_len);
            ...
            let scalar_decode_result = processor.decode_u64_array_scalar(&scalar_output, &mut scalar_decoded);
...
fn test_memory_comparison(processor: &SimdProcessor) {
    ...
    println!("Identical data - SIMD: {}, Scalar: {}", simd_result1, scalar_result1);
    println!("Different data - SIMD: {}, Scalar: {}", simd_result2, scalar_result2);
...
fn run_performance_benchmarks(processor: &SimdProcessor) {
    ...
    for size in data_sizes {
        println!("\nBenchmarking with {} bytes, {} iterations:", size, iterations);
        ...
        let simd_throughput = (size * iterations) as f64 / (1024.0 * 1024.0) / simd_time.as_secs_f64();
        let scalar_throughput = (size * iterations) as f64 / (1024.0 * 1024.0) / scalar_time.as_secs_f64();
        
        println!("  CRC64:");
        println!("    SIMD:   {:.2} MB/s ({:.2} ms)", simd_throughput, simd_time.as_millis());
        println!("    Scalar: {:.2} MB/s ({:.2} ms)", scalar_throughput, scalar_time.as_millis());
        ...
        let encode_speedup = scalar_encode_time.as_nanos() as f64 / simd_encode_time.as_nanos() as f64;
        let simd_encode_throughput = (values.len() * 8 * iterations) as f64 / (1024.0 * 1024.0) / simd_encode_time.as_secs_f64();
        let scalar_encode_throughput = (values.len() * 8 * iterations) as f64 / (1024.0 * 1024.0) / scalar_encode_time.as_secs_f64();
        
        println!("  U64 Encoding:");
        println!("    SIMD:   {:.2} MB/s ({:.2} ms)", simd_encode_throughput, simd_encode_time.as_millis());
        println!("    Scalar: {:.2} MB/s ({:.2} ms)", scalar_encode_throughput, scalar_encode_time.as_millis());
