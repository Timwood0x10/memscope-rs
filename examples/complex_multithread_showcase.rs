...
use memscope_rs::lockfree::{
    init_thread_tracker, finalize_thread_tracker,
    track_allocation_lockfree, track_deallocation_lockfree,
    export_comprehensive_analysis, SamplingConfig,
    ...
    match workload_type {
        WorkloadType::DataProcessing => {
            let result = execute_data_processing_workload(
                thread_id, should_track, &mut allocation_count, 
                &mut peak_memory_bytes, &mut operations_completed, completed_ops
            );
            if let Err(e) = result {
...
fn execute_batch_processing_workload(
    ...
    for batch in 0..batch_count {
        ...
        for i in 0..batch_size {
            ...
            let ptr = (thread_id as u64) << 32 | (batch as u64) << 16 | (i as u64) << 8 | size as u64;
...
fn execute_cache_worker_workload(
    ...
    for i in 0..base_iterations {
        let cache_key = i % 50;
        if !cache_entries.contains_key(&cache_key) {
            ...
            cache_entries.insert(cache_key, (ptr as usize, size));
            *allocation_count += 1;
            *peak_memory_bytes += size as u64;
        }
...
fn print_execution_summary(
    ...
    println!("\n📊 ========== EXECUTION SUMMARY ==========");
    println!("⏱️  Total execution time: {:.2} seconds", total_time.as_secs_f64());
    ...
    if let Ok(stats) = thread_stats.lock() {
        ...
        for stat in stats.iter() {
            let entry = workload_summary.entry(format!("{:?}", stat.workload_type))
        ...
        for (workload_type, (count, allocs, memory, time, ops)) in workload_summary {
            ...
            println!("   └─ Allocations: {} ({:.1} avg)", allocs, allocs as f64 / count as f64);
            println!("   └─ Peak Memory: {:.1} MB ({:.1} MB avg)", memory, memory / count as f64);
            println!("   └─ Execution: {:.1} ms ({:.1} ms avg)", time, time / count as f64);
            println!("   └─ Operations: {} ({:.1} avg)", ops, ops as f64 / count as f64);
        }
    }
    Ok(())
}
