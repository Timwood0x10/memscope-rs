...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   📄 JSON: {}/enhanced_fft_ecc_comprehensive.json",
        output_dir
    );
    println!("   🌐 HTML: {}/enhanced_fft_ecc_dashboard.html", output_dir);
    println!(
        "   📈 Rankings: {}/enhanced_fft_ecc_resource_rankings.json",
        output_dir
    );
    ...
    for (fft_size, thread_count, label) in fft_configs {
        ...
        pool.install(|| {
            ...
            for iteration in 0..3 {
                ...
                track_complex_allocation(
                    &fft_result1,
                    &format!("{}_fft1_iter{}", label, iteration),
                );

                let fft_result2 = parallel_fft(&signal2);
                track_complex_allocation(
                    &fft_result2,
                    &format!("{}_fft2_iter{}", label, iteration),
                );
                ...
                track_complex_allocation(
                    &correlation,
                    &format!("{}_correlation_iter{}", label, iteration),
                );
                ...
                track_complex_allocation(
                    &ifft_result,
                    &format!("{}_ifft_iter{}", label, iteration),
                );
                ...
                println!(
                    "     ⚡ Iteration {} completed - {} allocations tracked",
                    iteration + 1,
...
fn memory_intensive_ecc_workload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base_point = EllipticPoint::new(12345.67890, 67890.12345);

    // Generate large sets of scalars for parallel computation
    let scalar_sets: Vec<Vec<u64>> = (0..8)
        .map(|set_id| {
            (0..50)
                .map(|i| ((set_id + 1) * 10000 + i * 123) as u64)
                .collect()
        })
        .collect();
    ...
    scalar_sets
        ...
        .for_each(|(set_id, scalars)| {
            ...
            for (i, &scalar) in scalars.iter().enumerate() {
                ...
                let _k = (scalar * 2 + 0x123456789ABCDEF0) % (1u64 << 32);
...
fn concurrent_mixed_workload() -> Result<(), Box<dyn std::error::Error>> {
    ...
    workload_configs
        .into_par_iter()
        .for_each(|(name, fft_size, ec_scalar_base, _thread_count)| {
            let _ = init_thread_tracker(std::path::Path::new("./memoryanalysis"), None);
            ...
            for i in 0..5 {
                ...
                track_complex_allocation(
                    &modified_spectrum,
                    &format!("{}_modified_spectrum_{}", name, i),
                );
            }
        ...
        .for_each(|(count, size, label)| {
            ...
            for batch in 0..5 {
                ...
                for i in 0..count / 5 {
                    ...
                    track_complex_allocation(
                        &data,
                        &format!("{}_batch{}_alloc{}", label, batch, i),
                    );
                    allocations.push(data);
                }

                // Additional processing to create more allocations
                let processed: Vec<Vec<f64>> = allocations
                    .iter()
                    .map(|vec| vec.iter().map(|c| c.magnitude()).collect())
                    .collect();

                for (i, proc_data) in processed.iter().enumerate() {
                    track_f64_allocation(
                        proc_data,
                        &format!("{}_batch{}_processed{}", label, batch, i),
                    );
                }
            }
...
fn track_f64_allocation(data: &[f64], name: &str) {
    let ptr = data.as_ptr() as usize;
    let size_bytes = std::mem::size_of_val(data);
    let call_stack = vec![track_f64_allocation as *const () as usize, 0xCAFEBABE];
...
fn track_ec_points_allocation(data: &[EllipticPoint], name: &str) {
    let ptr = data.as_ptr() as usize;
    let size_bytes = std::mem::size_of_val(data);
    let call_stack = vec![track_ec_points_allocation as *const () as usize, 0xFEEDBEEF];
...
fn track_matrix_allocation(data: &[Vec<f64>], name: &str) {
    ...
    let call_stack = vec![track_matrix_allocation as *const () as usize, 0xABCDEF00];
...
fn parallel_ifft(input: &[Complex]) -> Vec<Complex> {
    ...
    // Conjugate the input
    let conjugated: Vec<Complex> = input
        .iter()
