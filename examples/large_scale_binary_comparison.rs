...
use memscope_rs::export::binary::{BinaryParser, detect_binary_type};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("User Binary Export: {:.2}ms", user_export_time.as_secs_f64() * 1000.0);
    println!("Full Binary Export: {:.2}ms", full_export_time.as_secs_f64() * 1000.0);
    println!("User Binary Parse: {:.2}ms", user_parse_time.as_secs_f64() * 1000.0);
    println!("Full Binary Parse: {:.2}ms", full_parse_time.as_secs_f64() * 1000.0);
    ...
    if full_parse_time.as_millis() < 300 {
        ...
        println!("Performance Target: MISSED ({}ms > 300ms target)", full_parse_time.as_millis());
    }
...
fn analyze_binary_files() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  File size: {} bytes ({:.2} KB)", user_info.file_size, user_info.file_size as f64 / 1024.0);
    ...
    println!("  File size: {} bytes ({:.2} KB)", full_info.file_size, full_info.file_size as f64 / 1024.0);
    ...
    println!("  Allocation ratio: {:.1}x more allocations", allocation_ratio);
    println!("  Count consistency: User={}, Full={}", 
    ...
    for (file_suffix, description) in &json_files {
        ...
        if let (Ok(user_content), Ok(full_content)) = (
            ...
            let (user_allocs, full_allocs) = if let (Some(user_data), Some(full_data)) = (
                ...
                let user_count = user_data.get("allocations")
                    .and_then(|a| a.as_array())
                    .map(|a| a.len())
                    .unwrap_or(0);
                let full_count = full_data.get("allocations")
                    .and_then(|a| a.as_array())
                    .map(|a| a.len())
                    .unwrap_or(0);
            ...
            println!("  File sizes: {} bytes (user) vs {} bytes (full)", user_size, full_size);
            if user_size > 0 {
                println!("  Size ratio: {:.1}x", full_size as f64 / user_size as f64);
            }
            println!("  Allocations: {} (user) vs {} (full)", user_allocs, full_allocs);
            println!("  Null fields: {} (user) vs {} (full)", user_nulls, full_nulls);
            
            if full_nulls == 0 {
                println!("  Null elimination: SUCCESS");
            } else {
                println!("  Null elimination: FAILED ({} nulls remaining)", full_nulls);
            }
        }
    }
    
    println!("\\nOverall JSON Analysis:");
    println!("  Total user JSON size: {} bytes ({:.2} KB)", user_total_size, user_total_size as f64 / 1024.0);
    println!("  Total full JSON size: {} bytes ({:.2} KB)", full_total_size, full_total_size as f64 / 1024.0);
    if user_total_size > 0 {
        println!("  Overall size ratio: {:.1}x", full_total_size as f64 / user_total_size as f64);
    }
