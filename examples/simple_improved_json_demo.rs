...
fn create_sample_memory_stats() -> MemoryStats {
    ...
    memory_stats.active_allocations = memory_stats
        .allocations
        .iter()
...
fn create_allocation_with_borrow_info(
    ptr: usize,
    size: usize,
    var_name: &str,
    type_name: &str,
...
fn create_allocation_with_clone_info(
    ptr: usize,
    size: usize,
    var_name: &str,
    type_name: &str,
...
fn create_allocation_with_full_info(
    ptr: usize,
    size: usize,
    var_name: &str,
    type_name: &str,
                ...
                "memory_analysis.json" => {
                    if let Some(allocations) = json_value["allocations"].as_array() {
                        if let Some(first_alloc) = allocations.first() {
                            ...
                            let has_ownership_flag =
                                first_alloc.get("ownership_history_available").is_some();
