...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if successful_threads == thread_count {
        ...
        println!(
            "⚠️  Partial success: {}/{} threads completed",
            successful_threads, thread_count
        );
    }
...
fn run_worker_thread(
    ...
    for i in 0..config.operation_count {
        ...
        match thread_idx % 4 {
            0 => {
                // Network threads: burst patterns
                if i % 200 == 0 {
                    thread::sleep(Duration::from_millis(2));
                }
            }
            1 => {
                // Data processor threads: steady processing
                if i % 150 == 0 {
                    thread::sleep(Duration::from_millis(1));
                }
            }
            2 => {
                // Cache threads: occasional heavy work
                if i % 300 == 0 {
                    thread::sleep(Duration::from_millis(3));
                }
            }
            _ => {
                // Log threads: very frequent, very quick
                if i % 50 == 0 {
    ...
    println!(
        "   Thread {}: {} allocs, {} deallocs, {:.1}% cleanup rate",
        thread_idx,
        total_allocations,
        deallocated_count,
...
fn generate_pure_analysis(output_dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut thread_allocs: Vec<_> = analysis
        .thread_stats
        .iter()
    ...
    for (i, (thread_id, allocs)) in thread_allocs.iter().take(5).enumerate() {
        ...
        println!(
            "   {}. Thread {}: {} allocs, {:.1}KB peak, avg {:.0}B",
            i + 1,
            thread_id,
            allocs,
