...
fn main() {
    ...
    println!(
        "🏗️ Created nested structure with {} items",
        nested_data.len()
    );
...
fn process_data() {
    ...
    println!(
        "📈 Temp processing: sum={}, length={}, box_sum={}",
        sum, length, box_sum
    );
...
fn process_hashmap(mut map: HashMap<String, Vec<i32>>) -> usize {
    ...
    // 创建一个新的 HashMap 进行转换
    let transformed: HashMap<String, usize> = map.into_iter().map(|(k, v)| (k, v.len())).collect();
...
fn create_large_buffer(size: usize) -> Vec<u8> {
    ...
    println!(
        "🔧 Created temporary buffers: {} and {} bytes",
        temp_buffer1.len(),
        temp_buffer2.len()
    );
