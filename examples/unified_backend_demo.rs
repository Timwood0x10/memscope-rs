...
use memscope_rs::unified::{
    UnifiedBackend, BackendConfig, TrackingStrategy, 
    detect_environment, unified_quick_start, test_unified_system
};
use std::thread;
...
fn demo_quick_start() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Analysis completed: {} bytes collected", analysis.raw_data.len());
...
fn demo_environment_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match environment {
        ...
        memscope_rs::unified::RuntimeEnvironment::MultiThreaded => {
            println!("💡 Recommendation: Use thread-local strategy for better scaling");
        }
        memscope_rs::unified::RuntimeEnvironment::AsyncRuntime(_) => {
...
async fn demo_manual_strategy() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔧 Configuration: Strategy=AsyncOptimized, SampleRate=80%, MaxOverhead=3%");
    
    let mut backend = UnifiedBackend::initialize(config)?;
    let session = backend.start_tracking()?;
    
    println!("✅ Custom backend initialized with session: {}", session.session_id());
    ...
    let analysis = backend.collect_data()?;
    println!("✅ Async analysis: {} bytes collected", analysis.raw_data.len());
...
fn demo_multi_threaded_tracking() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let analysis = backend.collect_data()?;
    println!("✅ Multi-threaded analysis: {} bytes collected", analysis.raw_data.len());
