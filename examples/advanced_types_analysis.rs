...
use memscope_rs::{init, track_var, analyze_advanced_types, get_global_tracker};
use std::cell::{Cell, RefCell};
use std::sync::{Arc, Mutex, RwLock, mpsc, atomic::{AtomicUsize, AtomicBool, Ordering}};
...
fn main() {
    ...
    println!("\n📦 Test 1: Interior Mutability Types");
    {
        ...
        let _tracked_refcell = track_var!(refcell_data.clone());
    ...
    println!("\n🔒 Test 2: Synchronization Primitives");
    {
        ...
        let _tracked_mutex = track_var!(mutex_data.clone());
        println!("✅ Created Arc<Mutex<String>>");
        
        let rwlock_data = Arc::new(RwLock::new(vec![10, 20, 30]));
        let _tracked_rwlock = track_var!(rwlock_data.clone());
        println!("✅ Created Arc<RwLock<Vec<i32>>>");
        
        // Demonstrate concurrent access
        let mutex_clone = mutex_data.clone();
        let rwlock_clone = rwlock_data.clone();
    ...
    println!("\n📡 Test 3: Channel Types");
    {
        ...
        let _tracked_sender = track_var!(sender.clone());
    ...
    println!("\n⚛️  Test 4: Atomic Types");
    {
        ...
        let _tracked_atomic = track_var!(atomic_counter.clone());
        println!("✅ Created AtomicUsize");
        
        let atomic_flag = Arc::new(AtomicBool::new(false));
        let _tracked_flag = track_var!(atomic_flag.clone());
        ...
        println!("   Final counter after concurrent increments: {}", 
    ...
    println!("\n🏗️ Test 6: Complex nested advanced types");
    {
        ...
        let _tracked_complex = track_var!(complex.clone());
    ...
    println!("\n📊 Analysis Results:");
    println!("==================");
    println!("Total advanced types analyzed: {}", analysis.statistics.total_advanced_types);
    ...
    if !analysis.by_category.is_empty() {
        ...
        for (category, types) in &analysis.by_category {
            ...
            for (i, type_info) in types.iter().take(3).enumerate() {
                println!("    {}. Behavior: interior_mut={}, thread_safe={}, can_block={}", 
                        i + 1,
    ...
    println!("\n📈 Performance Summary:");
    println!("======================");
    println!("Average overhead factor: {:.2}x", analysis.performance_summary.total_overhead_factor);
    println!("Total memory overhead: {} bytes", analysis.performance_summary.total_memory_overhead);
    println!("Lock-free types: {:.1}%", analysis.performance_summary.lock_free_percentage);
    println!("Dominant latency category: {:?}", analysis.performance_summary.dominant_latency_category);
    ...
    println!("   - Identified {} different categories of advanced types", analysis.by_category.len());
    println!("   - Detected {} potential issues", analysis.all_issues.len());
    println!("   - Average performance overhead: {:.1}x", analysis.performance_summary.total_overhead_factor);
    println!("   - {}% of types are lock-free", analysis.performance_summary.lock_free_percentage);
}