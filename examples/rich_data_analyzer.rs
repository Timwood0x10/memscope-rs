...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, (thread_id, stats)) in thread_performance.iter().take(10).enumerate() {
        ...
        println!(
            "   {}. Thread {}: {} allocs, {} deallocs ({:.1}% efficiency), {:.1}KB peak",
            i + 1,
            thread_id,
    ...
    for (i, hotstack) in analysis.hottest_call_stacks.iter().take(10).enumerate() {
        println!(
            "   {}. Hash 0x{:x}: {} occurrences, {:.1}KB total, Impact Score: {}",
            i + 1,
    ...
    println!("\n⚡ **MEMORY EFFICIENCY ANALYSIS**");
    let total_allocated_bytes: usize = analysis
        .thread_stats
        .values()
        .map(|s| s.total_allocated)
        .sum();
    let total_peak_bytes: usize = analysis.thread_stats.values().map(|s| s.peak_memory).sum();
    ...
    println!(
        ...
        total_peak_bytes as f64 / (1024.0 * 1024.0)
    );
    println!(
        "   Memory efficiency: {:.1}% (peak/total)",
    ...
    if leak_potential > 0 {
        println!(
            "   ⚠️  Potential memory leaks: {} unfreed allocations",
            leak_potential
        );
        println!(
            "   Leak rate: {:.1}%",
    ...
    if let Ok(entries) = std::fs::read_dir(&output_dir) {
        ...
        println!(
            "   Total data collected: {:.1} MB",
            collected_data_size as f64 / (1024.0 * 1024.0)
        );
        println!("   Binary event files: {}", bin_files);
        println!("   Frequency data files: {}", freq_files);
        println!(
            "   Average data per thread: {:.1} KB",
    ...
    println!("\n🎯 **DATA QUALITY ASSESSMENT**");
    let avg_allocs_per_thread =
        analysis.summary.total_allocations as f64 / analysis.thread_stats.len() as f64;
    let avg_unique_stacks =
        analysis.summary.unique_call_stacks as f64 / analysis.thread_stats.len() as f64;

    println!(
        "   Average allocations per thread: {:.0}",
        avg_allocs_per_thread
    );
    println!(
        "   Average unique call stacks per thread: {:.0}",
        avg_unique_stacks
    );
    ...
    println!(
        "   Call stack diversity score: {:.1}% (higher = more diverse patterns)",
        diversity_score
    );
    ...
    println!(
        "   • {} unique call stack patterns collected",
        analysis.summary.unique_call_stacks
    );
    println!(
        "   • {:.1}MB of binary event data generated",
        collected_data_size as f64 / (1024.0 * 1024.0)
    );
