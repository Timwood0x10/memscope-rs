...
fn demonstrate_basic_tracking(tracker: &MemoryTracker) -> TrackingResult<()> {
    ...
    tracing::info!(
        "Basic tracking stats: total={}, active={}, memory={}KB",
...
fn demonstrate_complex_structures(tracker: &MemoryTracker) -> TrackingResult<()> {
    ...
    tracker.associate_var(
        nested_ptr,
        "nested_structure".to_string(),
        "Vec<HashMap<&str, Vec<i32>>>".to_string(),
    )?;
    ...
    tracker.associate_var(
        cloned_ptr,
        "cloned_config".to_string(),
        "Arc<Vec<String>>".to_string(),
    )?;

    let stats = tracker.get_stats()?;
    tracing::info!(
        "Complex structures stats: total={}, active={}, memory={}KB",
        stats.total_allocations,
        stats.active_allocations,
        stats.active_memory / 1024
    );
...
fn demonstrate_lifecycle_analysis(tracker: &MemoryTracker) -> TrackingResult<()> {
    ...
    let stats = tracker.get_stats()?;
    tracing::info!(
        "Lifecycle analysis stats: total={}, active={}, peak={}KB",
        stats.total_allocations,
        stats.active_allocations,
...
fn demonstrate_export_functionality(
    ...
    if let Some(allocations) = parsed.get("allocations").and_then(|v| v.as_array()) {
        tracing::info!(
            "JSON export contains {} allocation records",
            allocations.len()
        );
...
fn demonstrate_performance_monitoring(tracker: &MemoryTracker) -> TrackingResult<()> {
    ...
    tracing::info!(
        "  Ops/sec: {:.2}",
        (allocation_count * 2) as f64 / duration.as_secs_f64()
    );
