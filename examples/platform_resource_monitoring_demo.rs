...
use memscope_rs::lockfree::{
    comprehensive_profile_execution, IntegratedProfilingSession, PlatformResourceCollector,
};
use std::sync::{
            ...
            match session.stop_profiling_and_analyze() {
                Ok(analysis) => {
                    ...
                    println!(
                        ...
                        analysis.memory_analysis.thread_stats.len()
                    );
                    println!(
                        "  Resource samples collected: {}",
                        analysis.resource_timeline.len()
                    );
...
fn io_intensive_thread(stop: Arc<AtomicBool>) {
    ...
    while !stop.load(Ordering::Relaxed) {
        ...
        for i in 0..3 {
            let temp_file =
                std::env::temp_dir().join(format!("memscope_io_{}_{}.tmp", file_counter, i));
