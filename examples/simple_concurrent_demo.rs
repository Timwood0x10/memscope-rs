...
fn test_multithreading() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Multi-threading completed in {:.2}ms", elapsed.as_secs_f64() * 1000.0);
...
fn test_thread_communication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let consumer = thread::spawn(move || {
        ...
        while let Ok(received_data) = rx.recv() {
            let processed = received_data.iter().map(|&x| x.wrapping_mul(2)).collect::<Vec<_>>();
            track_var!(processed);
            count += 1;
        }
        count
    ...
    let elapsed = start.elapsed();
    println!("✅ Thread communication completed in {:.2}ms, processed {} items", 
        elapsed.as_secs_f64() * 1000.0, processed_count);
...
fn test_parallel_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let elapsed = start.elapsed();
    println!("✅ Parallel processing completed in {:.2}ms", elapsed.as_secs_f64() * 1000.0);
...
fn show_final_stats(total_time: Duration) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(stats) = tracker.get_stats() {
        ...
        println!("✅ Export completed in {:.2}ms", export_time.as_secs_f64() * 1000.0);
    }
