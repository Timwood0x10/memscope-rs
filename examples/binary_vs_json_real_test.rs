...
impl PerformanceResult {
    ...
    fn print_detailed_results(&self) {
        ...
        println!("  💾 Data efficiency: {:.2} bytes per allocation", 
                 self.file_size as f64 / self.data_size as f64);
        
        // Calculate throughput metrics
        let total_time = self.export_time + self.import_time;
        let throughput_mb_per_sec = (self.file_size as f64 / 1024.0 / 1024.0) / total_time.as_secs_f64();
    ...
    fn compare_with_baseline(&self, baseline: &PerformanceResult) {
        ...
        let export_speedup = baseline.export_time.as_nanos() as f64 / self.export_time.as_nanos() as f64;
        let import_speedup = baseline.import_time.as_nanos() as f64 / self.import_time.as_nanos() as f64;
        ...
        println!("\n🎯 {} vs {} Comparison:", self.format_name, baseline.format_name);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("✅ Collected {} allocations from real tracking", test_data.allocations.len());
...
fn create_complex_test_data() {
    ...
    for i in 0..200 {
        ...
        track_var!(vec_data, format!("test_vector_{}", i));
        
        let map_data: HashMap<String, i32> = (0..20)
            .map(|j| (format!("key_{}_{}", i, j), j))
            .collect();
        track_var!(map_data, format!("test_hashmap_{}", i));
    }
    ...
    println!("  📈 Created ~650 tracked allocations with realistic patterns");
...
fn benchmark_json_performance(data: &TestExportData) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    // Export benchmark
...
fn benchmark_messagepack_performance(data: &TestExportData) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
    // Export benchmark
    let export_start = Instant::now();
...
fn benchmark_compressed_performance(data: &TestExportData) -> Result<PerformanceResult, Box<dyn std::error::Error>> {
...
fn print_comprehensive_analysis(json: &PerformanceResult, msgpack: &PerformanceResult, compressed: &PerformanceResult) {
    println!("\n🎯 Comprehensive Performance Analysis");
    println!("{}", "=".repeat(70));
    
    // Performance summary table
    println!("\n📊 Performance Summary:");
    println!("  Format              | Export    | Import    | Size      | Throughput");
    println!("  -------------------|-----------|-----------|-----------|------------");
    println!("  JSON               | {:>7.1}ms | {:>7.1}ms | {:>7.1}KB | {:>8.1}MB/s", 
             json.export_time.as_secs_f64() * 1000.0,
             json.import_time.as_secs_f64() * 1000.0,
             json.file_size as f64 / 1024.0,
             (json.file_size as f64 / 1024.0 / 1024.0) / (json.export_time + json.import_time).as_secs_f64());
    println!("  MessagePack        | {:>7.1}ms | {:>7.1}ms | {:>7.1}KB | {:>8.1}MB/s", 
             msgpack.export_time.as_secs_f64() * 1000.0,
             msgpack.import_time.as_secs_f64() * 1000.0,
             msgpack.file_size as f64 / 1024.0,
             (msgpack.file_size as f64 / 1024.0 / 1024.0) / (msgpack.export_time + msgpack.import_time).as_secs_f64());
    println!("  MessagePack + Zstd | {:>7.1}ms | {:>7.1}ms | {:>7.1}KB | {:>8.1}MB/s", 
             compressed.export_time.as_secs_f64() * 1000.0,
             compressed.import_time.as_secs_f64() * 1000.0,
             compressed.file_size as f64 / 1024.0,
             (compressed.file_size as f64 / 1024.0 / 1024.0) / (compressed.export_time + compressed.import_time).as_secs_f64());
