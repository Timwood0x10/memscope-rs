...
fn demonstrate_smart_pointers(tracker: &memscope_rs::tracker::Tracker) {
    ...
    println!("✓ Rc<Vec<i32>>: {} references", Rc::strong_count(&shared_data));
    ...
    println!("✓ Arc<String>: {} references", Arc::strong_count(&thread_safe));
...
fn demonstrate_complex_patterns(tracker: &memscope_rs::tracker::Tracker) {
    ...
    for i in 0..5 {
        ...
        for j in 0..10 { inner.insert(j, vec![format!("item_{}_{}", i, j); 3]); }
    ...
    for i in 0..1000 { computation.push(format!("Result {}: {}", i, i * i)); }
...
fn simulate_web_server_scenario(tracker: &memscope_rs::tracker::Tracker) {
    ...
    for i in 0..50 {
        sessions.insert(format!("sess_{i:08x}"), HashMap::from([
    ...
    for i in 0..200 {
        ...
        if request_log.len() > 100 { request_log.pop_front(); }
    }
...
fn simulate_data_processing_pipeline(tracker: &memscope_rs::tracker::Tracker) {
    ...
    for i in 0..500 { input_queue.push_back(format!("data_record_{i:06}")); }
    track!(tracker, input_queue);
    println!("✓ Input queue: 500 records");

    let mut stage1 = Vec::new();
    for i in 0..500 { stage1.push(format!("validated_{}", i)); }
    ...
    for i in 0..25 { errors.push(format!("Error {}: Processing failed", i)); }
