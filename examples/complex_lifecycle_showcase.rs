...
fn main() {
    ...
    println!("📌 Keeping {} variables alive for complete lifecycle analysis", _keep_alive.len());
}
...
fn demonstrate_builtin_types() -> Vec<Box<dyn std::any::Any>> {
    ...
    println!("✓ Small Vec: grew from capacity 5 to {} elements", small_vec.len());
    ...
    println!("✓ Large Vec: grew from capacity 1000 to {} elements", large_vec.len());
    ...
    println!("✓ Box<VecDeque>: {} items (front/back insertion)", boxed_vec_deque.len());
...
fn demonstrate_smart_pointers() -> Vec<Box<dyn std::any::Any>> {
    ...
    println!("✓ Rc<Vec<i32>>: {} references to shared data", Rc::strong_count(&shared_data));
    ...
    println!("✓ Arc<String>: {} references to thread-safe data", Arc::strong_count(&thread_safe_data));
...
fn demonstrate_custom_structures() -> Vec<Box<dyn std::any::Any>> { let mut keep_alive = Vec::new();
    ...
    user.preferences.insert("theme".to_string(), "dark".to_string());
    user.preferences.insert("language".to_string(), "en".to_string());
    user.preferences.insert("notifications".to_string(), "enabled".to_string());
    ...
    for i in 0..10 {
        ...
        db_conn.active_queries.push_back(format!("SELECT * FROM table_{}", i));
    }
    let boxed_db_conn = Box::new(db_conn);
    track_var!(boxed_db_conn).unwrap();
    println!("✓ Box<DatabaseConnection>: {} connections, {} active queries", 
             boxed_db_conn.connection_pool.len(), boxed_db_conn.active_queries.len());
...
fn demonstrate_complex_patterns() -> Vec<Box<dyn std::any::Any>> { let mut keep_alive = Vec::new();
    ...
    for i in 0..3000 {
        ...
        if i % 100 == 0 {
            ...
            if i % 500 == 0 {
                stream_processor.error_log.push(format!("Warning at byte {}", i));
            }
        }
    }
    let boxed_stream_processor = Box::new(stream_processor);
    track_var!(boxed_stream_processor).unwrap();
    println!("✓ Box<StreamProcessor>: buffer grew from 2KB to {} bytes, {} errors logged", 
             boxed_stream_processor.buffer.len(), boxed_stream_processor.error_log.len());
...
fn simulate_web_server_scenario() -> Vec<Box<dyn std::any::Any>> { let mut keep_alive = Vec::new();
    ...
    routes.insert("/api/users".to_string(), "UserController::index".to_string());
    routes.insert("/api/users/{id}".to_string(), "UserController::show".to_string());
    routes.insert("/api/posts".to_string(), "PostController::index".to_string());
    routes.insert("/api/auth/login".to_string(), "AuthController::login".to_string());
    ...
    for i in 0..50 {
        ...
        let session_data = HashMap::from([
            ...
            ("csrf_token".to_string(), format!("token_{:016x}", i * 12345)),
            ("last_activity".to_string(), format!("{}", 1640995200 + i * 60)),
        ]);
    ...
    println!("✓ Web server: {} routes, {} sessions, {} log entries", 
             boxed_routes.len(), boxed_sessions.len(), boxed_request_log.len());
    println!();
    keep_alive
}
...
fn simulate_data_processing_pipeline() -> Vec<Box<dyn std::any::Any>> { let mut keep_alive = Vec::new();
    ...
    for (i, record) in stage2_results.iter().enumerate() {
        ...
        final_results.entry(key).or_insert_with(Vec::new).push(record.clone());
    }
    ...
    for i in 0..25 {
        error_tracker.push(format!("Error {}: Processing failed for record {}", i, i * 20));
    }
    track_var!(error_tracker).unwrap();
    
    println!("✓ Data pipeline: {} → {} → {} groups, {} errors tracked",
             500, stage1_results.len(), boxed_final_results.len(), error_tracker.len());
    println!();
    keep_alive
}
...
fn generate_final_analysis() {
    ...
    if let Ok(stats) = tracker.get_stats() {
        ...
        println!("  • Active memory: {} bytes ({:.2} MB)", 
                 stats.active_memory, stats.active_memory as f64 / 1024.0 / 1024.0);
        ...
        println!("\nLifecycle Analysis:");
        println!("  • Average lifetime: {:.2}ms", lifecycle.average_lifetime_ms);
        println!("  • Completed allocations: {}", lifecycle.completed_allocations);
        println!("  • Memory growth events: {}", lifecycle.memory_growth_events);
        println!("  • Peak concurrent variables: {}", lifecycle.peak_concurrent_variables);
        println!("  • Memory efficiency: {:.2}%", lifecycle.memory_efficiency_ratio * 100.0);
        println!("  • Ownership transfers: {}", lifecycle.ownership_transfer_events);
        println!("  • Fragmentation score: {:.3}", lifecycle.fragmentation_score);
        
        println!("\nLifetime Distribution:");
        println!("  • Instant (< 1ms): {}", lifecycle.instant_allocations);
        println!("  • Short-term (1-100ms): {}", lifecycle.short_term_allocations);
        println!("  • Medium-term (100ms-1s): {}", lifecycle.medium_term_allocations);
        ...
        if !lifecycle.scope_metrics.is_empty() {
            ...
            for scope in &lifecycle.scope_metrics {
                println!("  • {}: {} vars, {:.1}ms avg, {:.1}% efficiency", 
                    scope.scope_name, 
        ...
        if !lifecycle.type_lifecycle_patterns.is_empty() {
            ...
            for pattern in &lifecycle.type_lifecycle_patterns {
                println!("  • {}: {:.1}x growth, {:?} ownership, {:?} risk", 
                    pattern.type_name, 
