//! Multi-Threaded Mode Test - Lockfree Tracker
//!
//! This example tests the multi-threaded mode using Lockfree Tracker.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔍 Multi-Threaded Mode Test (Lockfree Tracker)");
    println!("═════════════════════════════════════════════\n");

    let tracker = Arc::new(tracker!());
    let num_allocations = 50_000;
    let num_threads = 100;

    println!("📊 Configuration:");
    println!("  Mode: Multi-Threaded (Lockfree Tracker)");
    ...
    match tracker.export_json("multi_threaded_test") {
        Ok(_) => {
            println!("✅ Detailed JSON exported successfully!");
            println!("   📁 Files saved to MemoryAnalysis/multi_threaded_test/");
    ...
    match tracker.export_analysis("multi_threaded_analysis") {
        Ok(_) => {
            println!("✅ Analysis report exported successfully!");
            println!("   📁 File saved to MemoryAnalysis/multi_threaded_analysis_analysis.json");
