//! Async Mode Test - Async Tracker
//!
//! This example tests async mode using Async Tracker.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔍 Async Mode Test (Async Tracker)");
    println!("═══════════════════════════════════════════\n");

    let tracker = Arc::new(tracker!());
    let num_allocations = 20_000;
    let num_threads = 10;

    println!("📊 Configuration:");
    println!("  Mode: Async (Async Tracker)");
    ...
    match tracker.export_json("async_mode_test") {
        Ok(_) => {
            println!("✅ Detailed JSON exported successfully!");
            println!("   📁 Files saved to MemoryAnalysis/async_mode_test/");
    ...
    match tracker.export_analysis("async_mode_analysis") {
        Ok(_) => {
            println!("✅ Analysis report exported successfully!");
            println!("   📁 File saved to MemoryAnalysis/async_mode_analysis_analysis.json");
