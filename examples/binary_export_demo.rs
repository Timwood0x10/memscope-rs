...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let json_file = output_dir.join("binary_demo_example.json");
    ...
    println!("✅ Standard JSON conversion completed in {:?}", json_conversion_time);
    ...
    println!("🌐 HTML file: {} ({} bytes)", html_file.display(), html_size);
    ...
    let size_reduction = ((json_direct_size as f64 - binary_size as f64) / json_direct_size as f64) * 100.0;
    ...
    println!("  📁 Standard JSON size:     {} bytes (5 files)", json_direct_size);
    println!("  📁 Converted JSON size:    {} bytes (5 files)", total_json_size);
    println!("  📁 Single JSON size:       {} bytes (1 file)", single_json_size);
    ...
    println!("  • Total JSON files: {} (5 categorized + 1 single)", json_files.len() + 1);
    println!("  • Combined size: {} bytes", total_json_size);
    println!("  • Allocations found: {}", allocation_count);
    println!("  • Contains structured data: {}", json_content.contains("\"size\":"));
    ...
    println!("  • Contains allocation table: {}", html_content.contains("<table"));
    println!("  • Contains memory addresses: {}", html_content.contains("0x"));
    println!("  • Interactive dashboard: {}", html_content.contains("javascript") || html_content.contains("script"));
    ...
    println!("  • {} (binary format - {} bytes)", binary_file.display(), binary_size);
    for json_file_name in &json_files {
        let json_file_path = output_dir.join(json_file_name);
        if json_file_path.exists() {
    ...
    println!("  • {} (HTML report - {} bytes)", html_file.display(), html_size);
    println!("  • {} (single JSON - {} bytes)", single_json_file.display(), single_json_size);
    
    println!("\n💡 Next steps:");
    println!("  1. Open {} in your browser to view the interactive report", html_file.display());
    println!("  2. Examine {} to see the structured allocation data", first_json_file.display());
    println!("  3. Compare file sizes: binary ({} bytes) vs JSON ({} bytes)", binary_size, total_json_size);
...
"#,
        binary_file.file_name().unwrap().to_string_lossy(), binary_size,
        single_json_file.file_name().unwrap().to_string_lossy(), single_json_size,
        html_file.file_name().unwrap().to_string_lossy(), html_size,
        direct_json_dir.file_name().unwrap_or_default().to_string_lossy(), json_direct_size,
        speed_improvement, size_reduction,
        binary_export_time, json_direct_time,
    ...
    println!("  4. Read {} for detailed information", index_file.display());
