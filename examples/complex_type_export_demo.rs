...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("================================");
    println!(
        "⏱️  Standard export time: {:.2}ms",
        standard_time.as_millis()
    );
    println!(
        "⏱️  Optimized export time: {:.2}ms",
    ...
    let total_optimized_size =
        export_result.export_stats.main_file_size + export_result.export_stats.complex_files_size;

    println!("📏 Standard file size: {} bytes", standard_size);
    println!(
        "📏 Optimized main file: {} bytes",
        export_result.export_stats.main_file_size
    );
    println!(
        "📏 Complex type files: {} bytes",
    ...
    println!(
        "📉 Main file is {:.1}% of original size",
        size_ratio * 100.0
    );
...
fn demonstrate_selective_loading(
    ...
    if let Some(ref complex_file) = export_result.complex_types_file {
        if std::path::Path::new(complex_file).exists() {
            ...
            println!(
                "📖 Complex types file loaded on-demand: {} characters",
                complex_content.len()
            );
        }
    }
