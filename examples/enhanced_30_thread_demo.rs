...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ✅ Successful threads: {}/{}", 
             results.iter().filter(|(_, status)| *status == "Success").count(), 
             thread_count);
...
fn run_enhanced_thread(
    ...
    println!("   ✓ {} completed ({} ops)", config.name, config.operation_count);
    Ok(())
}
...
fn analyze_enhanced_data(output_dir: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Ok(entries) = std::fs::read_dir(output_dir) {
        ...
        println!("   💾 Total enhanced data: {:.1} MB", total_size as f64 / (1024.0 * 1024.0));
    }
