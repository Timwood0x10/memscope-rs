...
- **数据导出增强**：支持任务关系图导出
  ...
  - 节点包含 memory_usage 和 allocation_count 实际统计值
...
- **feat(task_registry)**: 添加 `TaskGuard` RAII 结构
  - `task_scope()` 方法返回 `TaskGuard`，自动管理任务生命周期
  - 自动设置 parent 关系（从当前 task_id 获取）
  - 简化 API：只需声明 registry，调用 task_scope，自动完成和导出
