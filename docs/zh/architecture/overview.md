...
> MemScope v0.1.10 — 九引擎流水线架构

---

## 系统架构

MemScope 通过 **九引擎流水线** 捕获、分析和可视化内存分配：
...
| **Unified** | `unified_tracker.rs` | 自动检测: CPU 数 → Core/Lockfree | "开箱即用" — 推荐默认 |
...
- **SVG** — 内存布局可视化
- **Binary** — 紧凑的 `.memscope` 格式 (比 JSON 快 80 倍)
...
| Lockfree | ~2-5% | ~64 字节/事件 | 线程本地 + try_lock |
