...
use crate::lockfree::analysis::{
    BottleneckType, LockfreeAnalysis, MemoryPeak, PerformanceBottleneck, ThreadInteraction,
    ThreadStats,
};
...
impl LockfreeAggregator {
    ...
    pub fn aggregate_all_threads(&self) -> Result<LockfreeAnalysis, Box<dyn std::error::Error>> {
        ...
        analysis.hottest_call_stacks = hottest_call_stacks
            .into_iter()
            .map(
                |(hash, freq, size)| crate::lockfree::analysis::HotCallStack {
    ...
    fn analyze_thread_data(
        ...
        let timeline: Vec<AllocationEvent> = sorted_events
            .into_iter()
    ...
    fn analyze_thread_interactions(
        &self,
        thread_stats: &HashMap<u64, ThreadStats>,
    ) -> Vec<ThreadInteraction> {
        ...
        for stats in thread_stats.values() {
            ...
            for (&hash, &frequency) in &stats.allocation_frequency {
                if frequency > 1000 && stats.avg_allocation_size < 1024.0 {
                    bottlenecks.push(PerformanceBottleneck {
                        ...
                        description: format!(
                            "High frequency ({}) small allocations (avg {}B) in thread {}",
                            frequency, stats.avg_allocation_size as usize, stats.thread_id
                        ),
                        suggestion: "Consider using memory pools or batch allocation strategies"
                            .to_string(),
                    });
                }
            }
            ...
            if stats.total_deallocations == 0 && stats.total_allocations > 100 {
                bottlenecks.push(PerformanceBottleneck {
                    ...
                    severity: 0.8,
                    description: format!(
                        "Potential memory leak: {} allocations, 0 deallocations in thread {}",
                        stats.total_allocations, stats.thread_id
                    ),
                    suggestion: "Review deallocation logic and consider using RAII patterns"
                        .to_string(),
                });
            }

            // Detect large allocation spikes
            if stats.peak_memory > 100 * 1024 * 1024 {
                // > 100MB
                bottlenecks.push(PerformanceBottleneck {
                    ...
                    description: format!(
                        "Large memory spike: {}MB peak in thread {}",
...
"#,
        ...
        for (thread_id, stats) in sorted_threads.iter().take(15) {
            // Show top 15 threads
        ...
        if analysis.performance_bottlenecks.is_empty() {
            html.push_str(
                r#"<div style="text-align: center; padding: 40px; color: #27ae60;">
        ...
        if analysis.thread_interactions.is_empty() {
            html.push_str(
                r#"<div style="text-align: center; padding: 40px; color: #7f8c8d;">
        ...
        html.push_str(&format!(
            ...
            if analysis.summary.total_allocations > 0 {
                analysis.summary.total_deallocations as f64
