...
use super::tracker::{init_thread_tracker, finalize_thread_tracker, SamplingConfig};
...
pub fn stop_tracing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_dir = OUTPUT_DIRECTORY.get()
        .ok_or("Output directory not set")?
        .clone();
    
    generate_reports(&output_dir)?;
    
    println!("🎉 Tracking complete: {}/memory_report.html", output_dir.display());
...
pub fn memory_snapshot() -> MemorySnapshot {
    ...
    MemorySnapshot {
        ...
        allocations: 0,
        deallocations: 0,
        active_threads: if TRACKING_ENABLED.load(Ordering::SeqCst) { 1 } else { 0 },
    }
}
...
fn print_analysis_summary(analysis: &super::analysis::LockfreeAnalysis) {
    ...
    if analysis.summary.total_allocations > 0 {
        let efficiency = analysis.summary.total_deallocations as f64 / 
                        analysis.summary.total_allocations as f64 * 100.0;
