...
impl StrategyFactory {
    ...
    pub fn record_performance(&mut self, strategy_name: &str, performance: StrategyPerformance) {
        ...
        let (avg_overhead_percent, success_rate) = {
            ...
            entry.avg_init_time_us =
                (1.0 - weight) * entry.avg_init_time_us + weight * performance.avg_init_time_us;
            entry.success_rate =
                (1.0 - weight) * entry.success_rate + weight * performance.success_rate;
            entry.satisfaction_score =
                (1.0 - weight) * entry.satisfaction_score + weight * performance.satisfaction_score;
        ...
        info!(
            "Performance recorded for {}: overhead={:.2}%, success={:.2}%",
            strategy_name,
    ...
    fn calculate_strategy_score(
        ...
        let overhead_score = if requirements.max_overhead_percent > 0.0 {
            (requirements.max_overhead_percent - performance.avg_overhead_percent).max(0.0)
                / requirements.max_overhead_percent
        } else {
