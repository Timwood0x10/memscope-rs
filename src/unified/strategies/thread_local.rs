...
impl ThreadLocalStrategy {
    ...
    pub fn register_current_thread(&self) -> Result<(), TrackerError> {
        ...
        debug!("Registering thread for tracking: id={}, name={:?}", thread_id, thread_name);
        ...
        THREAD_LOCAL_DATA.with(|data| {
            ...
            self.thread_registry.total_registered_threads.fetch_add(1, Ordering::Relaxed);
    ...
    pub fn track_allocation(
        ...
        THREAD_LOCAL_DATA.with(|data| {
            ...
            let global_id = self.global_state.next_allocation_id.fetch_add(1, Ordering::Relaxed);
            ...
            thread_data.thread_metrics.avg_allocation_time_ns = 
                (1.0 - weight) * thread_data.thread_metrics.avg_allocation_time_ns + 
                weight * allocation_time_ns;
            
            // Update global metrics atomically
            self.global_metrics.total_allocations.fetch_add(1, Ordering::Relaxed);
            self.global_metrics.total_bytes_allocated.fetch_add(size as u64, Ordering::Relaxed);
            
            debug!("Tracked allocation in thread {}: ptr={:x}, size={}, global_id={}", 
    ...
    fn collect_all_thread_data(&self) -> Result<Vec<ThreadLocalRecord>, TrackerError> {
        ...
        self.global_metrics.total_overhead_bytes.store(total_overhead, Ordering::Relaxed);
        
        // Sort records by global ID for consistent ordering
        all_records.sort_by_key(|r| r.global_id);
        
        info!("Collected {} records from threads, total overhead: {} bytes", 
              all_records.len(), total_overhead);
...
impl MemoryTracker for ThreadLocalStrategy {
    ...
    fn initialize(&mut self, config: TrackerConfig) -> Result<(), TrackerError> {
        debug!("Initializing thread-local strategy with config: {:?}", config);
        ...
        self.global_metrics.peak_concurrent_allocations.store(0, Ordering::Relaxed);
        self.global_metrics.total_overhead_bytes.store(0, Ordering::Relaxed);
        
        // Reset thread registry
        self.thread_registry.total_registered_threads.store(0, Ordering::Relaxed);
        self.thread_registry.active_tracking_threads.store(0, Ordering::Relaxed);
    ...
    fn stop_tracking(&mut self) -> Result<Vec<u8>, TrackerError> {
        ...
        let total_threads = self.thread_registry.total_registered_threads.load(Ordering::Relaxed);
        
        info!("Thread-local tracking stopped: {} allocations from {} threads", 
...
mod tests {
    ...
    fn test_thread_registration() {
        ...
        let total_threads = strategy.thread_registry.total_registered_threads.load(Ordering::Relaxed);
    ...
    fn test_json_export_format() {
        ...
        assert_eq!(first_alloc["tracking_strategy"].as_str().unwrap(), "thread_local");
    ...
    fn test_global_metrics_atomicity() {
        ...
        let global_allocs = strategy.global_metrics.total_allocations.load(Ordering::Relaxed);
        let global_bytes = strategy.global_metrics.total_bytes_allocated.load(Ordering::Relaxed);
