...
use crate::types::internal_types::{Allocation, Snapshot, Stats};
    ...
    fn analyze(&self, snapshot: &Snapshot) -> AnalysisReport {
        ...
        if leaked_allocs.is_empty() {
            results.push(AnalysisResult {
                ...
                details: "All allocations have been properly freed or have variable names".to_string(),
            ...
            if leaked_allocs.len() > self.config.max_leaks {
                results.push(AnalysisResult {
                    severity: Severity::Info,
                    message: format!("... and {} more leaks", leaked_allocs.len() - self.config.max_leaks),
...
impl Analyzer for FragmentationAnalyzer {
    ...
    fn analyze(&self, snapshot: &Snapshot) -> AnalysisReport {
        ...
        if active_allocs.is_empty() {
            ...
            let avg_size: f64 = active_allocs.iter().map(|a| a.size).sum::<usize>() as f64 / num_allocs as f64;
            let variance = active_allocs
                .iter()
                .map(|a| (a.size as f64 - avg_size).powi(2))
                .sum::<f64>() / num_allocs as f64;
            ...
            if small_allocs > 100 {
                results.push(AnalysisResult {
                    ...
                    details: "Consider pooling or batching small allocations to reduce fragmentation".to_string(),
                    related_pointers: Vec::new(),
                    suggestions: vec![
            ...
            results.push(AnalysisResult {
                ...
                details: format!(
                    ...
                    size_buckets[0], size_buckets[1], size_buckets[2], size_buckets[3], size_buckets[4]
                ),
                related_pointers: Vec::new(),
                suggestions: Vec::new(),
            });
        ...
        if allocations_with_lifetime.is_empty() {
            ...
            if short_term > allocations_with_lifetime.len() / 2 {
                results.push(AnalysisResult {
                    ...
                    details: format!("{}% of allocations are short-lived", (short_term * 100 / allocations_with_lifetime.len())),
                    related_pointers: Vec::new(),
                    suggestions: vec![
...
impl Analyzer for SafetyAnalyzer {
    ...
    fn analyze(&self, snapshot: &Snapshot) -> AnalysisReport {
        ...
        let mut unsafe_pointers = Vec::new();
