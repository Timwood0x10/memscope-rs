...
mod tests {
    ...
    fn test_variable_association() {
        ...
        // Associate variable
        tracker
            .associate_variable("test_var".to_string(), 1024)
            .unwrap();
    ...
    fn test_scope_guard() {
        ...
        // Use a block to ensure the guard is dropped
        {
            ...
            // Associate variable
            tracker
                .associate_variable("guard_var".to_string(), 512)
                .unwrap();
    ...
    fn test_concurrent_scope_operations() {
        ...
        let mut tracker_scope_names: Vec<String> =
            tracker_scopes.iter().map(|s| s.name.clone()).collect();
        ...
        for scope in tracker_scopes.iter() {
            assert!(
                scope.end_time.is_some(),
                "Scope {} has no end time",
                scope.name
            );
            assert!(
