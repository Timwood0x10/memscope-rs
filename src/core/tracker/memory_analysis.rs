...
mod tests {
    ...
    fn test_estimate_alignment() {
        ...
        assert_eq!(tracker.estimate_alignment("f64", 8), 8);
        assert_eq!(tracker.estimate_alignment("u32", 4), 4);
        assert_eq!(tracker.estimate_alignment("i32", 4), 4);
        assert_eq!(tracker.estimate_alignment("f32", 4), 4);
        assert_eq!(tracker.estimate_alignment("u16", 2), 2);
        assert_eq!(tracker.estimate_alignment("i16", 2), 2);
        ...
        assert_eq!(tracker.estimate_alignment("SomeStruct", 7), 4);
        assert_eq!(tracker.estimate_alignment("SomeStruct", 16), 8);
    }
    ...
    fn test_estimate_type_size() {
        ...
        assert_eq!(tracker.estimate_type_size("bool"), 1);
        assert_eq!(tracker.estimate_type_size("u16"), 2);
        assert_eq!(tracker.estimate_type_size("i16"), 2);
        assert_eq!(tracker.estimate_type_size("u32"), 4);
        assert_eq!(tracker.estimate_type_size("i32"), 4);
        assert_eq!(tracker.estimate_type_size("f32"), 4);
    ...
    fn test_analyze_memory_layout() {
        ...
        let layout = unknown_layout.unwrap();
        assert_eq!(layout.total_size, 16);
        assert!(!layout.field_layout.is_empty());
    }
    ...
    fn test_classify_container_type() {
        ...
        match other_type {
            ContainerType::Other { type_name } => {
                assert_eq!(type_name, "CustomStruct");
            }
            _ => panic!("Expected Other container type"),
        }
    }
    ...
    fn test_analyze_capacity_utilization() {
        ...
        let box_utilization = tracker.analyze_capacity_utilization(&box_type, 4);
        assert_eq!(box_utilization.utilization_ratio, 1.0);
    }
    ...
    fn test_detect_reallocation_patterns() {
        ...
        let box_patterns = tracker.detect_reallocation_patterns(&box_type, 4);
        match box_patterns.frequency_assessment {
            ReallocationFrequency::None => {
                // Expected for Box
            }
            _ => panic!("Expected no reallocations for Box"),
        }
    }
    ...
    fn test_calculate_container_efficiency_metrics() {
        ...
        let capacity_util = CapacityUtilization {
        ...
        let realloc_patterns = ReallocationPatterns {
            frequency_assessment: ReallocationFrequency::Moderate,
    ...
    fn test_analyze_generic_type() {
        ...
        let complex_info = complex_generic.unwrap();
        assert_eq!(complex_info.base_type, "HashMap");
        assert_eq!(complex_info.type_parameters.len(), 2);
    }
    ...
    fn test_collect_runtime_state() {
        ...
        assert!(runtime_state.cache_performance.l2_hit_rate >= 0.0);
        assert!(runtime_state.cache_performance.l2_hit_rate <= 1.0);
    ...
    fn test_analyze_type_relationships() {
        ...
        assert_eq!(rel.type_name, "Vec<String>");
        assert!(rel.complexity_score > 0);
        assert_eq!(rel.inheritance_depth, 0); // Rust doesn't have inheritance
        assert_eq!(rel.composition_breadth, 0);
    }
    ...
    fn test_track_type_usage() {
        ...
        assert!(info.performance_impact.memory_efficiency_score >= 0.0);
        assert!(info.performance_impact.memory_efficiency_score <= 100.0);
        assert!(info.performance_impact.cpu_efficiency_score >= 0.0);
    ...
    fn test_analyze_stack_allocation() {
        ...
        assert!(stack_info.is_some());
        let info = stack_info.unwrap();
        // Frame ID is always >= 0 for usize
        assert_eq!(info.var_name, "stack_var");
    ...
    fn test_extract_base_type() {
        ...
        assert_eq!(tracker.extract_base_type("Box<dyn Display>"), "Box");
    }
