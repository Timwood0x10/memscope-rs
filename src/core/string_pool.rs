...
impl PoolMetrics {
    ...
    fn record_intern(&self, string_len: usize, was_cached: bool) {
        ...
        if was_cached {
            ...
            self.duplicate_bytes_saved.fetch_add(string_len as u64, Ordering::Relaxed);
        } else {
            self.total_string_bytes.fetch_add(string_len as u64, Ordering::Relaxed);
        }
    }
}
...
impl StringPool {
    ...
    pub fn intern(&self, s: &str) -> Arc<str> {
        ...
        let result = self.pool.entry(s.to_string())
