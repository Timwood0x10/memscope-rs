...
mod tests {
    ...
    fn test_track_allocation_shared() {
        ...
        tracker
            .track_allocation_shared(ptr, size)
    ...
    fn test_get_active_allocations_shared() {
        ...
        tracker
            .track_allocation_shared(0x2000, 512)
            .expect("Failed to track allocation");
        tracker
            .track_allocation_shared(0x3000, 1024)
            .expect("Failed to track allocation");

        let shared_collection = tracker
            .get_active_allocations_shared()
            .expect("Failed to get active allocations");

        assert_eq!(shared_collection.len(), 2);

        // Check that allocations are present
        let ptrs: Vec<usize> = shared_collection.iter().map(|info| info.ptr()).collect();
    ...
    fn test_get_allocation_history_shared() {
        ...
        for i in 0..3 {
            tracker
                .track_allocation_shared(0x4000 + i * 0x100, 256)
                .expect("Failed to track allocation");
        }

        let history = tracker
            .get_allocation_history_shared()
    ...
    fn test_associate_var_shared() {
        ...
        tracker
            .track_allocation_shared(ptr, 1024)
            .expect("Failed to track allocation");

        // Then associate a variable with it
        tracker
            .associate_var_shared(ptr, "test_variable".to_string(), "TestType".to_string())
        ...
        let found = allocations.iter().find(|info| info.ptr() == ptr);
    ...
    fn test_clone_stats_retrieval() {
        ...
        tracker
            .track_allocation_shared(0x6000, 2048)
            .expect("Failed to track allocation");
    ...
    fn test_arc_sharing_efficiency() {
        ...
        // Track an allocation
        tracker
            .track_allocation_shared(0x7000, 1024)
            .expect("Failed to track allocation");

        // Get the allocation multiple times
        let collection1 = tracker
            .get_active_allocations_shared()
            .expect("Failed to get allocations");
        let collection2 = tracker
            .get_active_allocations_shared()
            .expect("Failed to get allocations");
    ...
    fn test_large_allocation_tracking() {
        ...
        tracker
            .track_allocation_shared(0x8000, large_size)
    ...
    fn test_allocation_history_persistence() {
        ...
        for i in 0..5 {
            tracker
                .track_allocation_shared(0x9000 + i * 0x100, 512)
                .expect("Failed to track allocation");
        }

        // Get history multiple times
        let history1 = tracker
            .get_allocation_history_shared()
            .expect("Failed to get history");
        let history2 = tracker
            .get_allocation_history_shared()
            .expect("Failed to get history");
