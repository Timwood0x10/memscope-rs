...
impl CloneMonitor {
    ...
    pub fn record_clone(&self, type_name: &str, size_bytes: usize, duration_ns: u64) {
        ...
        self.total_clone_time_ns.fetch_add(duration_ns, Ordering::Relaxed);
        
        // Update per-type statistics
        if let Ok(mut stats) = self.type_stats.lock() {
            let type_stat = stats.entry(type_name.to_string()).or_insert_with(|| {
                TypeCloneStats {
            ...
            let total_time = type_stat.avg_clone_time_ns * (type_stat.clone_count - 1) as f64 + duration_ns as f64;
    ...
    pub fn get_stats(&self) -> CloneMonitorStats {
        ...
        let performance_improvement = if total_clones > 0 {
            (optimized_clones + self.avoided_clones.load(Ordering::Relaxed)) as f64 / total_clones as f64
        } else {
            0.0
        };
