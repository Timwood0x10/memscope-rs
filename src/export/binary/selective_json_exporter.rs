...
use crate::export::binary::filter_engine::{FilterEngine, FilterEngineBuilder};
...
impl SelectiveJsonExporter {
    ...
    pub fn with_config(config: SelectiveJsonExportConfig) -> Result<Self, BinaryExportError> {
        let index_cache = IndexCache::with_config(config.index_cache_config.clone())?;
        let batch_processor = BatchProcessor::with_config(config.batch_processor_config.clone());
        let filter_engine = FilterEngineBuilder::new().build();
    ...
    pub fn export_to_json_selective<P: AsRef<Path>>(
        &mut self,
        binary_path: P,
        json_path: P,
        ...
        let read_options = SelectiveReadOptionsBuilder::new()
        ...
        // Start JSON document
        json_writer.write_header(index.total_records() as u64)?;
        
        // Process records in batches
        let mut processed_count = 0;
        let batch_size = self.config.batch_processor_config.batch_size;
        
        for batch_start in (0..index.total_records()).step_by(batch_size) {
            let batch_end = (batch_start + batch_size).min(index.total_records());
            ...
            let filtered_offsets = self.filter_engine.pre_filter_with_index(
                &batch_offsets,
                &index,
                filters,
            )?;
            ...
            let filtered_records = self.filter_engine.apply_precise_filters(
                &records.records,
                filters,
            )?;
    ...
    fn get_or_build_index<P: AsRef<Path>>(
        ...
        let index = index_builder.build_index(path)?;
        
        // Cache the index
        self.index_cache.put(path, &index)?;
        self.stats.index_cache_misses += 1;
    ...
    fn analyze_optimal_filters(
        ...
        match optimization_level {
            ...
            OptimizationLevel::Balanced => {
                ...
                filters.push(AllocationFilter::MinSize(32));
            }
            OptimizationLevel::Comprehensive => {
                // More aggressive filtering
                filters.push(AllocationFilter::MinSize(16));
                filters.push(AllocationFilter::ExcludeTemporary);
            }
        }
    ...
    fn update_derived_stats(&mut self) {
        if self.stats.total_export_time_us > 0 {
            self.stats.avg_export_throughput = (self.stats.total_allocations_exported as f64 * 1_000_000.0) 
