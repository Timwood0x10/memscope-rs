...
use crate::export::binary::index::{
    BinaryIndex, CompactAllocationIndex, QuickFilterData, RecordMetadata, StringTableIndex,
    BloomFilterParams,
};
...
impl QuickFilterBuilder {
    ...
    fn process_batch(&mut self) {
        ...
        let timestamp_min = self.current_batch.iter().map(|r| r.timestamp).min().unwrap_or(0);
        let timestamp_max = self.current_batch.iter().map(|r| r.timestamp).max().unwrap_or(0);
...
mod tests {
    ...
    fn test_build_index_from_binary_file() {
        ...
        let builder = BinaryIndexBuilder::new();
        let index = builder.build_index(temp_file.path()).unwrap();
    ...
    fn test_file_hash_computation() {
        ...
        let builder = BinaryIndexBuilder::new();
        let hash1 = builder.compute_file_hash(temp_file.path()).unwrap();
        let hash2 = builder.compute_file_hash(temp_file.path()).unwrap();
        
        // Same file should produce same hash
        assert_eq!(hash1, hash2);
        
        // Different content should produce different hash
        std::fs::write(temp_file.path(), b"different test data").unwrap();
        let hash3 = builder.compute_file_hash(temp_file.path()).unwrap();
