...
impl OptimizedRecord {
    ...
    fn format_field(&self, field: &AllocationField) -> Option<String> {
        match field {
            AllocationField::Ptr => {
                self.allocation.ptr.map(|ptr| format!("\"0x{:x}\"", ptr))
            }
            AllocationField::Size => {
                self.allocation.size.map(|size| size.to_string())
            }
            AllocationField::VarName => {
                self.allocation.var_name.as_ref().map(|var_name| {
                    match var_name {
            ...
            AllocationField::TypeName => {
                self.allocation.type_name.as_ref().map(|type_name| {
                    match type_name {
                        Some(name) => format!("\"{}\"", name.replace('"', "\\\"")),
                        None => "null".to_string(),
                    }
            ...
            AllocationField::ScopeName => {
                self.allocation.scope_name.as_ref().map(|scope_name| {
                    match scope_name {
                        Some(name) => format!("\"{}\"", name.replace('"', "\\\"")),
                        None => "null".to_string(),
                    }
            ...
            AllocationField::TimestampDealloc => {
                self.allocation.timestamp_dealloc.as_ref().map(|ts_opt| {
                    match ts_opt {
            ...
            AllocationField::ThreadId => {
                self.allocation.thread_id.as_ref().map(|thread_id| {
                    format!("\"{}\"", thread_id.replace('"', "\\\""))
                })
            }
            ...
            AllocationField::IsLeaked => {
                self.allocation.is_leaked.map(|leaked| {
                    if leaked { "true" } else { "false" }.to_string()
                })
            }
            AllocationField::StackTrace => {
                self.allocation.stack_trace.as_ref().map(|stack_trace_opt| {
                    match stack_trace_opt {
                        Some(trace) => {
                            let trace_json: Vec<String> = trace.iter()
            ...
            AllocationField::LifetimeMs => {
                self.allocation.lifetime_ms.as_ref().map(|lifetime_opt| {
                    match lifetime_opt {
...
impl StreamingFieldProcessor {
        ...
        F: FnMut(&OptimizedRecord) -> Result<(), BinaryExportError>,
    {
        ...
        for batch in allocations.chunks(self.config.batch_size) {
            for allocation in batch {
                self.process_record_streaming(allocation.clone(), requested_fields, &mut processor)?;
            }
    ...
    fn format_field_direct(&self, field: &AllocationField, allocation: &PartialAllocationInfo) -> Option<String> {
    ...
    fn evict_lru_entry(&mut self) -> Result<(), BinaryExportError> {
        if let Some((lru_key, lru_value)) = self.field_cache
            .iter()
...
mod tests {
    ...
    fn test_field_formatting() {
        ...
        assert_eq!(record.get_formatted_field(&AllocationField::Ptr), Some("\"0x1000\"".to_string()));
        assert_eq!(record.get_formatted_field(&AllocationField::Size), Some("1024".to_string()));
        assert_eq!(record.get_formatted_field(&AllocationField::VarName), Some("\"test_var\"".to_string()));
        assert_eq!(record.get_formatted_field(&AllocationField::IsLeaked), Some("false".to_string()));
    }
    ...
    fn test_preformatted_fields() {
        ...
        let mut record = OptimizedRecord::new(allocation);
        let fields = [AllocationField::Ptr, AllocationField::Size].into_iter().collect();
        
        record.preformat_fields(&fields);
        
        assert_eq!(record.preformatted_fields.len(), 2);
        assert!(record.preformatted_fields.contains_key(&AllocationField::Ptr));
        assert!(record.preformatted_fields.contains_key(&AllocationField::Size));
    }
    ...
    fn test_streaming_processing() {
        ...
        processor.process_record_streaming(allocation, &fields, |_record| {
            processed_count += 1;
            Ok(())
        }).unwrap();
    ...
    fn test_cache_operations() {
        ...
        // Test cache miss
        let result1 = processor.get_or_format_field("test_key", &AllocationField::Ptr, &allocation).unwrap();
        assert_eq!(result1, Some("\"0x1000\"".to_string()));
        assert_eq!(processor.get_stats().cache_misses, 1);
        assert_eq!(processor.cache_size(), 1);

        // Test cache hit
        let result2 = processor.get_or_format_field("test_key", &AllocationField::Ptr, &allocation).unwrap();
