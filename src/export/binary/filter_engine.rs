...
impl FilterStats {
    ...
    pub fn overall_efficiency(&self) -> f64 {
        if self.total_operations == 0 {
            ...
            let total_filtered = self.index_filtered_out + self.bloom_filtered_out + self.precise_filtered_out;
...
impl FilterEngine {
    ...
    fn apply_index_prefiltering(
        ...
        for &candidate_index in candidates {
            ...
            batch_candidates.entry(batch_index).or_default().push(candidate_index);
        }
        ...
        for filter in filters {
            match filter {
                ...
                AllocationFilter::TimestampRange(min, max) => {
                    ...
                    if let Some(&(batch_min, batch_max)) = quick_filter.timestamp_ranges.get(batch_index) {
                        if *max < batch_min || *min > batch_max {
                            return false; // No overlap, batch can't contain matches
                        }
                    }
        ...
        for filter in string_filters {
            match filter {
                ...
                AllocationFilter::ThreadContains(thread_id) => {
                    if !self.bloom_filter_might_contain(&quick_filter.thread_bloom_filter, thread_id) {
                        return false;
                    }
                }
    ...
    fn bloom_filter_might_contain(&self, bloom_filter: &[u8], value: &str) -> bool {
        ...
        let hash_functions = self.index.allocations.quick_filter_data
            .as_ref()
...
impl FilterOptimizer {
    ...
    pub fn optimize_filters(filters: &[AllocationFilter]) -> Vec<AllocationFilter> {
        ...
        optimized.sort_by(|a, b| {
            Self::filter_selectivity(a).partial_cmp(&Self::filter_selectivity(b))
...
mod tests {
    ...
    fn test_filter_optimizer() {
        ...
        let thread_equals_pos = optimized.iter().position(|f| {
            matches!(f, AllocationFilter::ThreadEquals(_))
        });
        let thread_contains_pos = optimized.iter().position(|f| {
            matches!(f, AllocationFilter::ThreadContains(_))
        });
    ...
    fn test_matches_all_filters() {
        ...
        let non_matching_filters = vec![
            AllocationFilter::ThreadEquals("worker".to_string()),
        ];
