...
mod tests {
    ...
    use crate::export::binary::variable_relationship_analyzer::{
        VariableRelationshipAnalyzer, NodeCategory, OwnershipStatus, RelationshipType,
        LifetimeCategory, EdgeDirection,
    };
    use std::io::Cursor;
    ...
    fn test_variable_relationship_analysis_integration() {
        ...
        let stack_nodes = analysis.graph.nodes.iter()
            .filter(|n| n.category == NodeCategory::Stack || n.category == NodeCategory::Primitive)
            .count();
        let collection_nodes = analysis.graph.nodes.iter()
            .filter(|n| n.category == NodeCategory::Collection)
            .count();
        let smart_pointer_nodes = analysis.graph.nodes.iter()
            .filter(|n| n.category == NodeCategory::SmartPointer)
            .count();
        let reference_nodes = analysis.graph.nodes.iter()
        ...
        assert!(analysis.summary.ownership_distribution.contains_key(&OwnershipStatus::Owner));
        ...
        let has_memory_adjacency = analysis.graph.links.iter()
    ...
    fn test_node_categorization() {
        let allocations = vec![
            create_test_allocation_with_relationships(0x1000, 4, Some("int_val"), Some("i32"), Some("main"), 1000, Some(10)),
            create_test_allocation_with_relationships(0x2000, 1024, Some("vec_val"), Some("Vec<u8>"), Some("main"), 1100, Some(100)),
            create_test_allocation_with_relationships(0x3000, 8, Some("box_val"), Some("Box<i32>"), Some("main"), 1200, Some(50)),
            create_test_allocation_with_relationships(0x4000, 8, Some("ref_val"), Some("&str"), Some("main"), 1300, Some(20)),
    ...
    fn test_ownership_status_detection() {
        let allocations = vec![
            create_test_allocation_with_relationships(0x1000, 4, Some("owned"), Some("i32"), Some("main"), 1000, Some(100)),
            create_test_allocation_with_relationships(0x2000, 8, Some("borrowed"), Some("&i32"), Some("main"), 1100, Some(50)),
            create_test_allocation_with_relationships(0x3000, 8, Some("mut_borrowed"), Some("&mut i32"), Some("main"), 1200, Some(30)),
            create_test_allocation_with_relationships(0x4000, 8, Some("shared"), Some("Rc<i32>"), Some("main"), 1300, Some(200)),
    ...
    fn test_lifetime_categorization() {
        let allocations = vec![
            create_test_allocation_with_relationships(0x1000, 4, Some("instant"), Some("i32"), Some("main"), 1000, Some(0)),
            create_test_allocation_with_relationships(0x2000, 4, Some("short"), Some("i32"), Some("main"), 1100, Some(50)),
            create_test_allocation_with_relationships(0x3000, 4, Some("medium"), Some("i32"), Some("main"), 1200, Some(500)),
            create_test_allocation_with_relationships(0x4000, 4, Some("long"), Some("i32"), Some("main"), 1300, Some(5000)),
            create_test_allocation_with_relationships(0x5000, 4, Some("persistent"), Some("i32"), Some("main"), 1400, Some(15000)),
    ...
    fn test_relationship_detection() {
        ...
        let allocations = vec![
            ...
            create_test_allocation_with_relationships(0x1000, 1024, Some("vec1"), Some("Vec<u8>"), Some("main"), 1000, Some(100)),
            create_test_allocation_with_relationships(0x2000, 2048, Some("vec2"), Some("Vec<u8>"), Some("main"), 1100, Some(200)),
            
            // Adjacent memory allocations (should have memory adjacency)
            create_test_allocation_with_relationships(0x3000, 512, Some("buf1"), Some("Vec<u8>"), Some("main"), 1200, Some(150)),
            create_test_allocation_with_relationships(0x3000 + 512, 256, Some("buf2"), Some("Vec<u8>"), Some("main"), 1300, Some(100)),
            
            // Same scope allocations (should have containment)
            create_test_allocation_with_relationships(0x4000, 4, Some("var1"), Some("i32"), Some("helper"), 1400, Some(50)),
        ...
        // Should have memory adjacency relationships
        let memory_adjacency_count = analysis.graph.links.iter()
        ...
        let containment_count = analysis.graph.links.iter()
            .filter(|edge| edge.relationship == RelationshipType::Containment)
            .count();
        assert!(containment_count > 0);

        // Should have temporal relationships (consecutive allocations)
        let temporal_count = analysis.graph.links.iter()
    ...
    fn test_d3js_compatibility() {
        let allocations = vec![
            create_test_allocation_with_relationships(0x1000, 1024, Some("node1"), Some("Vec<u8>"), Some("main"), 1000, Some(100)),
        ...
        for edge in &analysis.graph.links {
            ...
            assert!(matches!(edge.direction, EdgeDirection::Directed | EdgeDirection::Undirected | EdgeDirection::Bidirectional));
        }
    ...
    fn test_memory_insights() {
        let allocations = vec![
            create_test_allocation_with_relationships(0x1000, 1024, Some("small"), Some("i32"), Some("main"), 1000, Some(10)),
            create_test_allocation_with_relationships(0x2000, 2048, Some("medium"), Some("Vec<u8>"), Some("main"), 1100, Some(500)),
        ...
        assert!(insights.lifetime_distribution.contains_key(&LifetimeCategory::Short));
        assert!(insights.lifetime_distribution.contains_key(&LifetimeCategory::Medium));
        assert!(insights.lifetime_distribution.contains_key(&LifetimeCategory::Persistent));
