...
/// Binary export implementation - removed to avoid conflicts with main tracker
// impl MemoryTracker {
    ...
    pub fn load_selective_binary<P: AsRef<Path>>(
        path: P,
        criteria: SelectionCriteria,
    ) -> TrackingResult<Vec<AllocationInfo>> {
        let export_data = Self::load_from_binary(path)?;
        ...
        if let Some(index) = &export_data.header.index {
            ...
            let filtered: Vec<AllocationInfo> = export_data.allocations
                .into_iter()
