...
use crate::export::fast_export_coordinator::{FastExportCoordinator, CompleteExportStats};
use crate::export::quality_validator::{QualityValidator, ValidationResult, ExportConfig, ExportModeManager, ExportMode, ValidationTiming};
...
pub fn export_with_validation<P: AsRef<Path>>(
    ...
    Box::pin(async move {
        ...
        println!("✅ Export completed: {} allocations, {:.2} MB", 
                stats.parallel_processing.total_allocations,
                stats.write_performance.total_bytes_written as f64 / 1024.0 / 1024.0);
...
impl ExportCoordinator {
    ...
    pub fn update_config(&mut self, mut config: ExportConfig, data_size: Option<usize>) -> Vec<String> {
        let mut warnings = config.validate_and_fix();
        
        if let Some(size) = data_size {
            let (optimized_config, optimization_warnings) = self.mode_manager.optimize_config(config, size);
...
mod tests {
    ...
    fn test_mode_manager_auto_selection() {
        ...
        let small_mode = manager.determine_optimal_mode(1024); // 1KB
        // Large data should use fast mode  
        let large_mode = manager.determine_optimal_mode(50 * 1024 * 1024); // 50MB
        
        // Note: This depends on the manager's default mode being Auto
        let auto_manager = ExportModeManager::with_settings(ExportMode::Auto, 10 * 1024 * 1024, 5000);
        assert_eq!(auto_manager.determine_optimal_mode(1024), ExportMode::Slow);
        assert_eq!(auto_manager.determine_optimal_mode(50 * 1024 * 1024), ExportMode::Fast);
    }
}