...
impl MemoryTracker {
    ...
    pub fn load_simple_binary<P: AsRef<Path>>(
        path: P,
    ) -> TrackingResult<SimpleBinaryData> {
        let path = path.as_ref();
        ...
        // Deserialize (try MessagePack first, then bincode)
        let export_data: SimpleBinaryData = rmp_serde::from_slice(&decompressed_data)
            .or_else(|_| {
                bincode::deserialize(&decompressed_data)
                    .map_err(|e| TrackingError::SerializationError(format!("Bincode error: {}", e)))
            })
            .map_err(|e| TrackingError::SerializationError(format!("Deserialization failed: {}", e)))?;
...
mod tests {
    ...
    fn test_simple_allocation_info_serialization() {
        ...
        let deserialized: SimpleAllocationInfo = bincode::deserialize(&bincode_data).unwrap();
        assert_eq!(alloc.ptr, deserialized.ptr);
        assert_eq!(alloc.size, deserialized.size);
    }
}