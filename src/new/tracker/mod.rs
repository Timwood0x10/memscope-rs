...
<body>
    ...
    /// Export only user-defined variables to binary format (migrated from MemoryTracker::export_user_binary)
    pub fn export_user_binary<P: AsRef<std::path::Path>>(&self, path: P) -> Result<(), String> {
        let snapshot = self.snapshot();

        // Filter to user-defined variables only (allocations with var_name)
        ...
        let user_snapshot = crate::types::internal_types::Snapshot {
            allocations: user_allocations,
            ..snapshot
        };

        self.export_to_binary_internal(&user_snapshot, path)
    }

    /// Export all allocations (user + system) to binary format (migrated from MemoryTracker::export_full_binary)
    pub fn export_full_binary<P: AsRef<std::path::Path>>(&self, path: P) -> Result<(), String> {
        let snapshot = self.snapshot();

        tracing::info!(
            "Exporting {} total allocations to binary format",
            snapshot.allocations.len()
        );

        self.export_to_binary_internal(&snapshot, path)
    }

    /// Internal method to export snapshot to binary
    fn export_to_binary_internal<P: AsRef<std::path::Path>>(
        &self,
        snapshot: &crate::types::internal_types::Snapshot,
        path: P,
    ) -> Result<(), String> {
        use crate::export::exporter::{BinaryExporter, ExportBackend, ExportConfig};
        use std::fs;
    ...
    /// Parse binary file to standard JSON format (migrated from MemoryTracker::parse_binary_to_standard_json)
    pub fn parse_binary_to_standard_json<P: AsRef<std::path::Path>>(
        binary_path: P,
        base_name: &str,
    ) -> Result<(), String> {
        // This is a simplified implementation
        // Full implementation would need to parse binary format and generate 4 separate JSON files
        tracing::info!(
            "Parsing binary file to standard JSON: {:?}",
            binary_path.as_ref()
        );

        // Read binary file
        let data = std::fs::read(binary_path).map_err(|e| e.to_string())?;
        ...
        let snapshot_data =
            match serde_json::from_slice::<crate::types::internal_types::Snapshot>(&data) {
                ...
                Err(_) => {
                    return Err("Failed to parse binary file".to_string());
                }
            };

        // Write the main JSON file
        let main_json_path = format!("{}.json", base_name);
        std::fs::write(&main_json_path, snapshot_data).map_err(|e| e.to_string())?;

        tracing::info!("Generated standard JSON file: {}", main_json_path);
        Ok(())
    }

    /// Export interactive lifecycle timeline (migrated from MemoryTracker::export_lifecycle_timeline)
    pub fn export_lifecycle_timeline<P: AsRef<std::path::Path>>(
        ...
        tracing::info!(
            "Generating lifecycle timeline for {} allocations",
            snapshot.allocations.len()
        );

        // Create a simple SVG timeline visualization
        let mut svg_content = String::from("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n");
        svg_content
            .push_str("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 1200 800\">\n");
        svg_content.push_str("    <style>\n");
        svg_content
            .push_str("        .allocation { fill: #4CAF50; stroke: #2E7D32; stroke-width: 1; }\n");
        svg_content
            .push_str("        .leaked { fill: #F44336; stroke: #C62828; stroke-width: 1; }\n");
        svg_content
            .push_str("        .label { font-family: Arial, sans-serif; font-size: 12px; }\n");
        svg_content.push_str("        .title { font-family: Arial, sans-serif; font-size: 20px; font-weight: bold; }\n");
        svg_content.push_str("    </style>\n");
        svg_content.push_str("    <rect width=\"100%\" height=\"100%\" fill=\"#f5f5f5\"/>\n");
        svg_content.push_str(
            "    <text x=\"20\" y=\"40\" class=\"title\">Memory Lifecycle Timeline</text>\n",
        );

        // Add allocation blocks
        let mut y = 80;
        for (i, allocation) in snapshot.allocations.iter().take(50).enumerate() {
            let x = 50 + (i * 20) % 1100;
            let width = 30;
            let height = 20;
            let is_leaked = allocation.free_ts.is_none();
            let class = if is_leaked { "leaked" } else { "allocation" };
            ...
            if let Some(ref var_name) = allocation.meta.var_name {
                let display_name = if var_name.len() > 15 {
                    format!("{}...", &var_name[..15])
                } else {
                    var_name.clone()
                };
        ...
        std::fs::write(path_ref, svg_content).map_err(|e| e.to_string())?;

        tracing::info!("Lifecycle timeline exported to: {:?}", path_ref);
        Ok(())
    }
