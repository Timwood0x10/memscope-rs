...
impl ExportBackend for BinaryExporter {
    ...
    fn export(&self, snapshot: &Snapshot, _config: &ExportConfig) -> ExportResult<ExportOutput> {
        // Use JSON fallback for now since bincode 2.x has Encode trait issues
        let json = serde_json::to_vec(snapshot)
            .map_err(|e| ExportError::Serialization(format!("JSON fallback failed: {e}")))?;

        Ok(ExportOutput::Binary(json))
    }
...
impl HtmlGenerator {
    fn generate_html(&self, snapshot: &Snapshot, _config: &ExportConfig) -> ExportResult<String> {
        ...
        html.push_str("                        <tbody>\n");
        html.push_str(
            "                            <!-- Allocation rows will be inserted here -->\n",
        );
        html.push_str("                        </tbody>\n");
        html.push_str("                    </table>\n");
        html.push_str("                </div>\n");
        ...
        html.push_str(
            "                totalSize: data.allocations.reduce((sum, a) => sum + a.size, 0),\n",
        );
        html.push_str(
            "                activeAllocations: data.allocations.filter(a => !a.free_ts).length,\n",
        );
        html.push_str(
            "                leakedAllocations: data.allocations.filter(a => !a.free_ts).length,\n",
        );
        html.push_str("            };\n");
        ...
        html.push_str(
            "                    <div class=\"kpi-value\">${kpis.totalAllocations}</div>\n",
        );
        html.push_str("                    <div class=\"kpi-label\">Total Allocations</div>\n");
        html.push_str("                </div>\n");
        html.push_str("                <div class=\"kpi-card\">\n");
        html.push_str(
            "                    <div class=\"kpi-value\">${formatBytes(kpis.totalSize)}</div>\n",
        );
        html.push_str("                    <div class=\"kpi-label\">Total Size</div>\n");
        html.push_str("                </div>\n");
        html.push_str("                <div class=\"kpi-card\">\n");
        html.push_str(
            "                    <div class=\"kpi-value\">${kpis.activeAllocations}</div>\n",
        );
        html.push_str("                    <div class=\"kpi-label\">Active Allocations</div>\n");
        html.push_str("                </div>\n");
        html.push_str("                <div class=\"kpi-card\">\n");
        html.push_str(
            "                    <div class=\"kpi-value\">${kpis.leakedAllocations}</div>\n",
        );
