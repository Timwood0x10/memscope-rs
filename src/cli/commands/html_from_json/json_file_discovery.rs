...
impl JsonFileDiscovery {
    ...
    pub fn discover_files(&self) -> Result<JsonDiscoveryResult, JsonDiscoveryError> {
        ...
        if !input_path.exists() || !input_path.is_dir() {
            return Err(JsonDiscoveryError::DirectoryNotFound(self.input_dir.clone()));
        }
        ...
        for config in file_configs {
            match self.find_file_for_config(&config) {
                Ok(Some(file_info)) => {
                    ...
                    if let Some(max_size_mb) = config.max_size_mb {
                        ...
                        if file_info.size_bytes > max_bytes {
                            println!("⚠️  File {} ({:.1} MB) exceeds size limit ({} MB)", 
                ...
                Ok(None) => {
                    if config.required {
                        println!("❌ Required file not found: {}_{}*.json", self.base_name, config.suffix);
                        missing_required.push(config);
                    } else {
                        println!("⚠️  Optional file not found: {}_{}*.json (skipping)", self.base_name, config.suffix);
                    }
                }
                Err(e) => {
        ...
        println!("   Total size: {:.1} MB", total_size_bytes as f64 / 1024.0 / 1024.0);
        ...
        if !missing_required.is_empty() {
            let missing_names: Vec<String> = missing_required.iter()
        ...
        if !oversized_files.is_empty() {
            let oversized_names: Vec<String> = oversized_files.iter()
    ...
    fn find_file_for_config(&self, config: &JsonFileConfig) -> Result<Option<JsonFileInfo>, std::io::Error> {
        // Try exact match first
        let exact_path = format!("{}/{}_{}.json", self.input_dir, self.base_name, config.suffix);
    ...
    fn create_file_info(&self, file_path: &str, config: &JsonFileConfig) -> Result<JsonFileInfo, std::io::Error> {
...
mod tests {
    ...
    fn test_directory_not_found() {
        ...
        assert!(matches!(result, Err(JsonDiscoveryError::DirectoryNotFound(_))));
    }
