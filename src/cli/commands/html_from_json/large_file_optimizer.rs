    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            LargeFileError::FileTooLarge(size, limit) => {
                write!(f, "File size ({} bytes) exceeds limit ({} bytes)", size, limit)
            }
            LargeFileError::MemoryLimitExceeded(used, limit) => {
                write!(f, "Memory usage ({} bytes) exceeds limit ({} bytes)", used, limit)
            }
...
impl MemoryMonitor {
    ...
    pub fn allocate(&self, size: usize) -> Result<(), LargeFileError> {
        ...
        if new_usage > self.memory_limit {
            return Err(LargeFileError::MemoryLimitExceeded(new_usage, self.memory_limit));
        }
    ...
    pub fn get_stats(&self) -> MemoryStats {
        ...
        MemoryStats {
            ...
            efficiency_ratio: if peak > 0 { current as f64 / peak as f64 } else { 1.0 },
        }
    }
}
...
impl LargeFileOptimizer {
    ...
    pub fn process_file<P: AsRef<Path>>(
        ...
        if file_size > self.config.max_file_size_bytes {
            return Err(LargeFileError::FileTooLarge(file_size, self.config.max_file_size_bytes));
        }
        
        println!("🔧 Processing large file: {} ({:.1} MB)", 
            path.display(), file_size as f64 / 1024.0 / 1024.0);
        ...
        println!("✅ File processed: {:.1} MB/s, {} objects, {}ms", 
    ...
    fn process_streaming<P: AsRef<Path>>(
        ...
        self.memory_monitor.allocate(self.config.stream_chunk_size)?;
        
        // For streaming, we'll read the JSON in chunks and validate structure
        let mut buffer = String::new();
        reader.read_to_string(&mut buffer).map_err(LargeFileError::IoError)?;
        ...
        self.memory_monitor.deallocate(self.config.stream_chunk_size);
        ...
        match file_type {
            "memory_analysis" => {
                ...
                if !obj.contains_key("allocations") && !obj.contains_key("summary") {
                    return Err(LargeFileError::ValidationError(
                        "Memory analysis JSON must contain 'allocations' or 'summary' field".to_string()
                    ));
                }
            }
            "unsafe_ffi" => {
                ...
                if !obj.contains_key("enhanced_ffi_data") && !obj.contains_key("summary") {
                    return Err(LargeFileError::ValidationError(
                        "Unsafe FFI JSON must contain 'enhanced_ffi_data' or 'summary' field".to_string()
                    ));
                }
            }
            "performance" => {
                ...
                let obj = json.as_object().unwrap();
                if !obj.contains_key("memory_performance") && !obj.contains_key("allocation_distribution") {
                    return Err(LargeFileError::ValidationError(
...
mod tests {
    ...
    fn test_process_small_file() {
        ...
        let test_data = r#"{"allocations": [{"ptr": "0x123", "size": 100}], "summary": {"total": 1}}"#;
    ...
    fn test_json_validation() {
        ...
        assert!(optimizer.validate_json_structure(&valid_json, "memory_analysis").is_ok());
        
        // Test invalid memory analysis JSON
        let invalid_json = serde_json::json!({
            "invalid_field": "value"
        });
        assert!(optimizer.validate_json_structure(&invalid_json, "memory_analysis").is_err());
    }
}