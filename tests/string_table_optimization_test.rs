...
fn test_string_table_compression_effectiveness() {
    ...
    export_to_binary_with_config(&allocations, temp_file_optimized.path(), &config_optimized)
        .unwrap();
    ...
    assert!(
        size_ratio < 1.5,
        "Optimized file should not be more than 50% larger than unoptimized"
    );
}
...
fn test_string_table_with_unique_strings() {
    ...
    assert!(
        result.is_ok(),
        "Export should succeed even with unique strings"
    );
