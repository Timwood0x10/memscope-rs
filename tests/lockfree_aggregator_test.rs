        ...
        .map(|thread_idx| {
            ...
            thread::spawn(move || -> Result<(), String> {
                ...
                for alloc_idx in 0..allocations_per_thread {
                    let allocation_result = generate_thread_allocations(
                        thread_idx, alloc_idx, &operation_counter
                    );
...
fn validate_data_generation(
    ...
    assert!(file_count >= config.expected_min_files,
            "Insufficient files generated: expected >= {}, got {}", 
            config.expected_min_files, file_count);
    
    assert!(total_file_size > 1024, 
            "Total file size too small: {} bytes", total_file_size);
}
...
fn test_aggregation_functionality(output_dir: &std::path::Path, config: &AggregatorTestConfig) {
    ...
    assert_eq!(analysis.thread_stats.len(), config.thread_count,
               "Thread count mismatch in analysis");
    
    assert!(analysis.summary.total_allocations > 0,
            "No allocations found in analysis");
    
    assert!(analysis.summary.unique_call_stacks > 0,
            "No unique call stacks found");
    
    // Validate thread statistics
    for (thread_id, stats) in &analysis.thread_stats {
        assert!(stats.total_allocations > 0,
                "Thread {} has no allocations", thread_id);
        
        assert!(stats.allocation_frequency.len() > 0,
                "Thread {} has no frequency data", thread_id);
        
        assert!(stats.timeline.len() > 0,
                "Thread {} has no timeline events", thread_id);
    }
    ...
    assert!(analysis.hottest_call_stacks.len() > 0,
...
fn test_report_generation(output_dir: &std::path::Path) {
    ...
    let json_size = std::fs::metadata(&json_path)
        .expect("Failed to get JSON metadata").len();
    assert!(json_size > 100, "JSON report too small: {} bytes", json_size);
    ...
    let html_size = std::fs::metadata(&html_path)
        .expect("Failed to get HTML metadata").len();
    assert!(html_size > 1000, "HTML report too small: {} bytes", html_size);
    
    // Validate HTML content structure
    let html_content = std::fs::read_to_string(&html_path)
        .expect("Failed to read HTML report");
    
    assert!(html_content.contains("<html>"), "Invalid HTML structure");
    assert!(html_content.contains("Thread Statistics"), "Missing thread statistics section");
    assert!(html_content.contains("Performance Bottlenecks"), "Missing bottlenecks section");
    assert!(html_content.contains("Hottest Call Stacks"), "Missing hot call stacks section");
}
...
fn validate_performance_requirements(test_start: Instant) {
    ...
    assert!(total_duration.as_secs() < 30,
            "Test took too long: {:?}", total_duration);
...
fn test_aggregator_error_handling() {
    ...
    assert!(result.is_ok(), "Aggregator should handle missing directory gracefully");
...
fn test_sampling_configuration_effectiveness() {
    ...
    for config in configs {
        assert!(config.validate().is_ok(), "Sampling configuration should be valid");
    }
