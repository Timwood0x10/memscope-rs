...
use memscope_rs::export::binary_exporter::{
    BinaryExportOptions, BinaryExporter, ValidationConfig, ValidationLevel,
};
...
fn setup_corrupted_test_file(
    ...
    let test_file_path = temp_dir
        .path()
        .join(format!("{}_corrupted.memscope", test_name));
...
mod tests {
    ...
    fn test_magic_number_corruption() {
        ...
        assert!(
            error_detected,
            "Strict parser should detect magic number corruption"
        );
        assert!(
            error_message.contains("magic") || error_message.contains("Invalid"),
            "Error message should mention magic number issue: {}",
            error_message
        );
    }
    ...
    fn test_checksum_validation() {
        ...
        assert!(
            error_detected,
            "Parser with checksum validation should detect corruption"
        );
        assert!(
            recovery_successful,
            "Parser without checksum validation should still work"
        );
    }
    ...
    fn test_truncated_file_handling() {
        ...
        let test_file_path = setup_corrupted_test_file(
            &temp_dir,
            &tracker,
            "truncated_test",
            ErrorSimulation::TruncatedFile,
    ...
    fn test_invalid_version_handling() {
        ...
        assert!(
            error_message.contains("version") || error_message.contains("Unsupported"),
    ...
    match recovery_result {
        Ok(_) => {
            ...
            if let Some(stats) = recovered_stats {
                println!(
                    "   Recovered stats: {} total allocations",
...
fn test_error_message_accuracy() {
    ...
    for error_type in error_types {
        ...
        let test_file_path =
            setup_corrupted_test_file(&temp_dir, &tracker, &test_name, error_type.clone())
                .expect("Failed to setup corrupted test file");
        ...
        match result {
            Err(e) => {
                ...
                match error_type {
                    ...
                    ErrorSimulation::InvalidCompression => {
                        assert!(
                            error_msg.contains("compression") || error_msg.contains("Invalid"),
                    ...
                    ErrorSimulation::TruncatedFile => {
                        assert!(
                            error_msg.contains("size")
                                ...
                                || error_msg.contains("small")
                                || error_msg.contains("directory"),
                            "Truncation error should be descriptive: {}",
                            error_msg
                        );
                    }
                    _ => {}
...
fn test_data_integrity_validation() {
    ...
    let options = BinaryExportOptions::default().validation(ValidationConfig {
    ...
    tracker
        .export_to_binary_with_options(&binary_path, options)
...
fn test_recovery_limits() {
    ...
    let test_file_path = temp_dir
        .path()
        .join("recovery_limits_test_corrupted.memscope");
    std::fs::copy(&actual_binary_path, &test_file_path).expect("Failed to copy file");
    ...
    println!(
        "Limited recovery (2 attempts): {}",
        if limited_result.is_ok() {
            "Success"
        } else {
            "Failed"
        }
...
fn test_malformed_header_recovery() {
    ...
    let test_file_path = temp_dir
        .path()
        .join("malformed_header_test_corrupted.memscope");
    std::fs::copy(&actual_binary_path, &test_file_path).expect("Failed to copy file");
...
fn test_file_corruption_simulation() {
    ...
    for (pattern_name, corruption_fn) in corruption_patterns {
        let binary_path = temp_dir
            .path()
            .join(format!("corruption_{}.bin", pattern_name.replace(" ", "_")));
        tracker
            .export_to_binary(&binary_path)
            .expect("Export failed");
        ...
        let test_file_path = temp_dir.path().join(format!(
            "corruption_{}_corrupted.memscope",
...
fn test_advanced_partial_recovery() {
    ...
    let test_data = vec![
        (
            "Critical data",
            vec![(0x1000, 1024, "Vec<i32>"), (0x2000, 2048, "String")],
        ),
        (
            "Optional data",
            vec![
                (0x3000, 512, "HashMap<String,String>"),
                (0x4000, 256, "Option<Box<Data>>"),
            ],
        ),
        (
            "Bulk data",
            (0..50)
                .map(|i| (0x10000 + i * 64, 64, "BulkItem"))
                .collect(),
        ),
    ];
    ...
    for (level_name, corruption_ratio) in corruption_levels {
        ...
        let corrupted_path = temp_dir.path().join(format!(
            "recovery_{}_{}.bin",
...
fn test_comprehensive_checksum_validation() {
    ...
    for (test_name, corruption_fn) in checksum_tests {
        ...
        let test_path = temp_dir
            .path()
            .join(format!("checksum_{}.memscope", test_name.replace(" ", "_")));
        ...
        println!(
            "  {}: Checksum validation={}, No validation={}",
            test_name,
        ...
        if checksum_result.is_ok() {
            ...
            println!(
                "    ✅ Checksum validation detected corruption in {}",
                test_name
            );
        }
    }
...
fn test_detailed_error_reporting() {
    ...
    for (scenario_name, setup_fn, expected_keywords) in error_scenarios {
        let test_path = temp_dir
            .path()
            .join(format!("error_{}.bin", scenario_name.replace(" ", "_")));

        // First create a valid file, then corrupt it
        if scenario_name != "Empty file" && scenario_name != "Invalid magic" {
            let temp_export_path = temp_dir
                .path()
                .join(format!("temp_{}.bin", scenario_name.replace(" ", "_")));
            tracker
                .export_to_binary(&temp_export_path)
                .expect("Export failed");
        ...
        match result {
            Err(e) => {
                ...
                let keyword_matches: Vec<_> = expected_keywords
                    .iter()
            ...
            Ok(_) => {
                panic!(
                    "Expected error for scenario '{}' but parsing succeeded",
                    scenario_name
                );
            }
        }
...
fn test_pattern_based_recovery() {
    ...
    let patterns = vec![
        (
            "Sequential",
            (0..20)
                .map(|i| (0x1000 + i * 64, 64, format!("Item{}", i)))
                .collect::<Vec<_>>(),
        ),
        (
            "Sparse",
            vec![
                (0x1000, 64, "First".to_string()),
                (0x10000, 128, "Middle".to_string()),
                (0x100000, 256, "Last".to_string()),
            ],
        ),
        (
            "Clustered",
            (0..10)
                .map(|i| (0x2000 + i * 16, 16, format!("Small{}", i)))
                .collect::<Vec<_>>(),
        ),
        (
            "Mixed sizes",
            vec![
                (0x3000, 1024, "Large".to_string()),
                (0x4000, 16, "Tiny".to_string()),
                (0x5000, 512, "Medium".to_string()),
            ],
        ),
    ];
    ...
    for (pattern_name, allocations) in patterns {
        ...
        let pattern_path = temp_dir
            .path()
            .join(format!("pattern_{}.bin", pattern_name.replace(" ", "_")));
        tracker
            .export_to_binary(&pattern_path)
            .expect("Export failed");

        // Get actual file path and copy for testing
        let actual_pattern_path = get_actual_binary_path(&pattern_path);
        let test_pattern_path = temp_dir.path().join(format!(
            "pattern_{}_corrupted.memscope",
            pattern_name.replace(" ", "_")
        ));
...
mod integration_error_tests {
    ...
    fn test_comprehensive_error_scenarios() {
        ...
        let scenarios = vec![
            ...
            (
                "Magic corruption",
                Some(ErrorSimulation::CorruptMagicNumber),
            ),
            ("Header corruption", Some(ErrorSimulation::CorruptHeader)),
            (
                "Section corruption",
                Some(ErrorSimulation::CorruptSectionData),
            ),
            (
                "Checksum corruption",
                Some(ErrorSimulation::CorruptChecksum),
            ),
        ...
        for (scenario_name, corruption) in scenarios {
            let export_path = temp_dir
                .path()
                .join(format!("{}.bin", scenario_name.replace(" ", "_")));
            tracker
                .export_to_binary(&export_path)
                .expect("Export failed");
            ...
            let file_path = temp_dir
                .path()
                .join(format!("{}_test.memscope", scenario_name.replace(" ", "_")));
        ...
        let valid_results: Vec<_> = results
            .iter()
            .filter(|r| r.test_name.contains("Valid file"))
            .collect();
        assert!(
            valid_results.iter().all(|r| r.recovery_successful),
            "All parsers should handle valid files successfully"
        );

        let corruption_results: Vec<_> = results
            .iter()
            .filter(|r| !r.test_name.contains("Valid file"))
            .collect();
        let strict_detections = corruption_results
            .iter()
            .filter(|r| r.test_name.contains("Strict") && r.error_detected)
            .count();
        let recovery_successes = corruption_results
            .iter()
            .filter(|r| r.test_name.contains("Recovery") && r.recovery_successful)
            .count();

        println!("\nStatistics:");
        println!(
            "  Strict parser detections: {}/{}",
            strict_detections,
            corruption_results.len() / 3
        );
        println!(
            "  Recovery parser successes: {}/{}",
            recovery_successes,
            corruption_results.len() / 3
        );
        ...
        assert!(
            strict_detections > 0,
            "Strict parser should detect some corruptions"
        );
    ...
    fn test_error_handling_under_stress() {
        ...
        for (config_name, options) in stress_configs {
            ...
            assert!(
                duration.as_secs() < 30,
                "Error handling should complete within 30 seconds"
            );
        }
