...
fn run_multithreaded_test(config: &TestConfig) -> Result<(), Box<dyn std::error::Error>> {
    info!(
        "Running test with {} threads, {} allocations per thread",
        ...
        .map(|thread_idx| {
            ...
            thread::spawn(move || -> Result<(), String> {
                ...
                for allocation_idx in 0..allocations_per_thread {
                    ...
                    // Track allocation
                    track_allocation_lockfree(ptr, size, &call_stack).map_err(|e| e.to_string())?;
                ...
                finalize_thread_tracker().map_err(|e| e.to_string())?;
    ...
    info!(
        "Test completed in {:?} with {} successful threads",
        test_duration, completed_threads
    );
    Ok(())
}
...
fn validate_test_results(
    ...
    if completion_rate < 0.95 {
        return Err(format!(
            "Thread completion rate too low: {:.2}%",
    ...
    if operation_rate < 0.90 {
        return Err(format!(
            "Operation completion rate too low: {:.2}%",
    ...
    if duration > config.test_duration_limit {
        warn!(
            "Test duration exceeded limit: {:?} > {:?}",
            duration, config.test_duration_limit
        );
    }
    ...
    if thread_files_found < completed_threads {
        return Err(format!(
            "Missing thread files: found {}, expected {}",
            thread_files_found, completed_threads
        )
        .into());
    }

    info!(
        "Validation successful: {:.1}% threads completed, {:.1}% operations completed",
        completion_rate * 100.0,
        operation_rate * 100.0
    );
    Ok(())
...
fn generate_analysis_report(
    ...
    info!(
        "  - Hottest call stacks: {}",
        analysis.hottest_call_stacks.len()
    );
    info!(
        "  - Thread interactions: {}",
    ...
    let high_severity_bottlenecks = analysis
    ...
    if high_severity_bottlenecks > 0 {
        warn!(
            "Found {} high-severity performance bottlenecks",
