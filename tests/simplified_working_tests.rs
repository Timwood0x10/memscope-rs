...
fn test_basic_memory_tracking_fixed() {
    ...
    assert!(
        ...
        "Expected at least 1 allocation, got {}",
        stats.total_allocations
    );
    assert!(
        stats.active_allocations >= 1,
        "Expected at least 1 active allocation, got {}",
        stats.active_allocations
    );
    assert!(
        stats.active_memory > 0,
        "Expected positive active memory, got {}",
        stats.active_memory
    );
    ...
    assert_eq!(
        ...
        "Expected 0 active allocations after deallocation, got {}",
        stats.active_allocations
    );
    assert_eq!(
        stats.active_memory, 0,
        "Expected 0 active memory after deallocation, got {}",
        stats.active_memory
    );
}
...
fn test_binary_export_and_import_with_extensions() {
    ...
    assert_eq!(
        imported_allocations[0].var_name,
        Some("test_var1".to_string())
    );
...
fn test_binary_to_html_conversion_with_clean_template() {
    ...
    assert!(
        html_content.contains("enhanced_var"),
        "HTML should contain variable name"
    );
    assert!(
        html_content.contains("Vec<String>"),
        "HTML should contain type name"
    );
    ...
    assert!(
        html_content.contains("class="),
        "HTML should contain CSS classes"
    );
    assert!(
    ...
    assert!(
        html_content.contains("Memory Analysis Report"),
        "HTML should contain report title"
    );
    assert!(
        html_content.len() > 1000,
        "HTML should be substantial in size"
    );

    // Verify improve.md extensions are in HTML
    assert!(
        html_content.contains("borrow_info"),
        "HTML should contain borrow_info data"
    );
    assert!(
        html_content.contains("clone_info"),
        "HTML should contain clone_info data"
    );
    assert!(
        html_content.contains("immutable_borrows"),
        "HTML should contain borrow details"
    );
}
...
fn test_concurrent_tracking_fixed() {
    ...
    if stats.active_allocations > 0 {
        assert!(
            stats.active_memory > 0,
            "Should have active memory if active allocations exist"
        );
    }
}
...
fn test_improve_md_extensions_serialization() {
    ...
    assert_eq!(
        allocation.borrow_info.as_ref().unwrap().immutable_borrows,
        deserialized.borrow_info.as_ref().unwrap().immutable_borrows
    );
    assert_eq!(
        allocation.clone_info.as_ref().unwrap().clone_count,
        deserialized.clone_info.as_ref().unwrap().clone_count
    );
    assert_eq!(
        allocation.ownership_history_available,
        deserialized.ownership_history_available
    );
    ...
    assert_eq!(
        allocation.ownership_history_available,
        imported_allocation.ownership_history_available
    );
}
...
fn test_html_converter_functionality() {
    ...
    // Verify content
    assert!(
        html_content.contains("HTML Converter Test"),
        "Should contain project name"
    );
    assert!(
        html_content.contains("converter_test_var"),
        "Should contain variable name"
    );
    assert!(
        html_content.contains("HashMap<String, Vec<i32>>"),
        "Should contain type name"
    );
    assert!(html_content.contains("2048"), "Should contain size");
    assert!(
        html_content.contains("test_function"),
    ...
    assert!(
        html_content.contains("immutable_borrows"),
        "Should contain borrow info"
    );
    assert!(
        html_content.contains("clone_count"),
        "Should contain clone info"
    );
    ...
    assert!(
        ...
        "Should have stats grid styling"
    );
    assert!(
        html_content.contains("allocations-table"),
        "Should have allocations table"
    );
}
