...
mod export_functionality_tests {
    ...
    fn test_json_export_functionality() {
        ...
        assert!(
            !json_files.is_empty(),
            "At least one JSON file should exist in: {}",
            project_dir.display()
        );
        ...
        assert!(
            json_content.contains("allocations"),
            "JSON should contain allocations array"
        );
        assert!(
            json_content.contains("json_var1"),
            "JSON should contain first variable"
        );
        assert!(
            json_content.contains("json_var2"),
            "JSON should contain second variable"
        );
    }
    ...
    fn test_export_with_empty_data() {
        ...
        assert!(
            json_content.contains("allocations"),
            "Should contain allocations data structure"
        );
        ...
        let html_path = std::path::Path::new("MemoryAnalysis/empty_project")
            .join("empty_project_user_dashboard.html");
        let alt_html_path =
            std::path::Path::new("MemoryAnalysis").join("empty_project_user_dashboard.html");
        ...
        assert!(
            html_content.contains("<!DOCTYPE html>"),
            "Should be valid HTML even when empty"
        );
    }
    ...
    fn test_large_dataset_export() {
        ...
        assert!(
            json_content.contains("allocations")
                || json_content.contains("variables")
                || json_content.contains("large_var"),
            "Should contain allocation data. Content preview: {}",
            &json_content[..std::cmp::min(200, json_content.len())]
        );
        ...
        assert!(
            metadata.len() > 1000,
            "Large dataset JSON should be substantial size"
        );
        assert!(
            metadata.len() < 10_000_000,
            "Large dataset JSON should not be excessively large"
        );
    }
}
