...
fn test_binary_to_html_generation() {
    ...
    assert!(
        html_content.contains("Memory Analysis Report")
            || html_content.contains("Memory & FFI Snapshot Analysis")
    );
    ...
    assert!(
        html_content.contains("Total Allocations") || html_content.contains("total-allocations")
    );
...
fn test_json_compatibility_with_existing_format() {
    ...
    if let (Value::Array(from_binary), Value::Array(direct)) = (&parsed_from_binary, &parsed_direct)
    {
        ...
        for (binary_item, direct_item) in from_binary.iter().zip(direct.iter()) {
            ...
            assert_eq!(
                binary_item["timestamp_alloc"],
                direct_item["timestamp_alloc"]
            );
...
fn test_performance_comparison() {
    ...
    assert!(
        ...
        "Binary export should be at least as fast as JSON"
    );
...
fn test_memory_tracker_integration() {
    ...
    tracker
        .fast_track_allocation(0x5000, 1024, "integration_test_1".to_string())
        .unwrap();
    tracker
        .fast_track_allocation(0x6000, 2048, "integration_test_2".to_string())
        .unwrap();
    tracker
        .fast_track_allocation(0x7000, 512, "integration_test_3".to_string())
        .unwrap();
    ...
    assert!(
        result.is_ok(),
        "MemoryTracker binary export failed: {:?}",
        result
    );
    ...
    assert!(
        memory_analysis_dir.exists(),
        "MemoryAnalysis directory should be created"
    );
                    ...
                    for sub_entry in sub_entries {
                        if let Ok(sub_entry) = sub_entry {
                            if sub_entry.path().extension()
                                == Some(std::ffi::OsStr::new("memscope"))
                            {
    ...
    assert!(
        metadata.len() > 16,
        "Binary file should have content beyond header"
    );
    ...
    let json_result =
        memscope_rs::core::tracker::MemoryTracker::parse_binary_to_json(&binary_file, &json_path);
    assert!(
        json_result.is_ok(),
        "Binary to JSON conversion failed: {:?}",
        json_result
    );

    let html_result =
        memscope_rs::core::tracker::MemoryTracker::parse_binary_to_html(&binary_file, &html_path);
    assert!(
        html_result.is_ok(),
