...
fn test_basic_functionality_regression() {
    ...
    assert!(tracker
        .associate_var(0x1000, "test_var1".to_string(), "i32".to_string())
        .is_ok());
    assert!(tracker
        .associate_var(0x2000, "test_var2".to_string(), "String".to_string())
        .is_ok());
...
fn test_export_functionality_regression() {
    ...
    assert!(tracker
        .associate_var(0x3000, "export_test".to_string(), "i32".to_string())
        .is_ok());
...
fn test_analysis_functionality_regression() {
    ...
    for i in 0..3 {
        ...
        assert!(tracker
            .associate_var(ptr, format!("test_var_{}", i), "i32".to_string())
            .is_ok());
    }
    ...
    for thread_id in 0..2 {
        ...
        let handle = thread::spawn(move || {
            for i in 0..3 {
                ...
                let _ = tracker_clone.associate_var(
                    ptr,
...
fn test_memory_safety_regression() {
    ...
    for i in 0..50 {
        ...
        if i % 10 == 0 {
            assert!(tracker
                .associate_var(ptr, format!("safety_test_{}", i), "u64".to_string())
                .is_ok());
        }
...
pub fn run_regression_tests() -> bool {
    ...
    let test_functions = vec![
        (
            "Basic Functionality",
            test_basic_functionality_regression as fn(),
        ),
        ("Tracking Macros", test_tracking_macros_regression as fn()),
        ("Trackable Types", test_trackable_types_regression as fn()),
        (
            "Export Functionality",
            test_export_functionality_regression as fn(),
        ),
        (
            "Analysis Functionality",
            test_analysis_functionality_regression as fn(),
        ),
        (
            "Utility Functions",
            test_utility_functions_regression as fn(),
        ),
        (
            "Concurrent Access",
            test_concurrent_access_regression as fn(),
        ),
