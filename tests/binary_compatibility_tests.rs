...
impl DataEquivalenceChecker {
        ...
        match (binary_val, direct_val) {
            ...
            _ => {
                result.add_difference(
                    path,
                    format!("Value mismatch: {:?} != {:?}", binary_val, direct_val),
                );
            }
        }
    }
        ...
        for key in all_keys {
            ...
            match (binary_obj.get(&key), direct_obj.get(&key)) {
                ...
                (Some(_), None) => {
                    result.add_difference(
                        &current_path,
                        "Key exists in binary but not in direct JSON".to_string(),
                    );
                }
                (None, Some(_)) => {
                    result.add_difference(
                        &current_path,
                        "Key exists in direct but not in binary JSON".to_string(),
                    );
                }
    ...
    fn compare_numbers(
        ...
        if let (Some(b_f64), Some(d_f64)) = (binary_num.as_f64(), direct_num.as_f64()) {
            ...
            let relative_diff = if d_f64.abs() > 0.0 {
                diff / d_f64.abs()
            } else {
                diff
            };
            ...
            if relative_diff > self.tolerance {
                result.add_difference(
                    path,
                    format!(
                        "Number difference too large: {} vs {} (diff: {:.6})",
                        b_f64, d_f64, relative_diff
                    ),
                );
            }
            ...
            result.add_difference(
                path,
                format!("Number mismatch: {:?} != {:?}", binary_num, direct_num),
            );
        }
    }
}
...
impl EquivalenceResult {
    ...
    pub fn print_summary(&self) {
        if self.is_equivalent {
            ...
            println!(
                "❌ Data equivalence check failed with {} differences:",
...
mod tests {
    ...
    fn test_json_output_equivalence() {
        ...
        tracker
            .export_to_json(&direct_json_path)
            .expect("Direct JSON export failed");

        // The actual JSON file will be created at MemoryAnalysis/direct/direct_memory_analysis.json
        let actual_direct_json_path = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("direct")
            .join("direct_memory_analysis.json");
    ...
    fn test_api_compatibility() {
        ...
        println!(
            "Tracker stats: total_allocations={}, active_allocations={}",
        ...
        tracker
            .export_to_json_with_options(&json_with_options_path, ExportOptions::default())
        ...
        let json_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test")
            .join("api_test_memory_analysis.json");
        let json_options_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test_options")
            .join("api_test_options_memory_analysis.json");

        let binary_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test.memscope");
        let binary_options_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test_options.memscope");
        let fast_binary_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test_fast.memscope");
        let comprehensive_binary_actual = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("api_test_comprehensive.memscope");

        // Check if files exist - use the actual paths from the export system
        // First, let's debug what files actually exist
        println!(
            "Checking for files in temp directory: {}",
    ...
    fn test_version_compatibility() {
        ...
        let actual_binary_path = temp_dir
            .path()
            .join("MemoryAnalysis")
            .join("version_test.memscope");

        // Test parsing with different parser options
        let strict_parser = BinaryParser::with_options(BinaryParserOptions::strict());

        let recovery_parser = BinaryParser::with_options(BinaryParserOptions::recovery_mode());

        let fast_parser = BinaryParser::with_options(BinaryParserOptions::fast());
...
fn test_cross_platform_consistency() {
    ...
    tracker
        .export_to_binary_with_options(&little_endian_path, options)
...
fn test_format_validation() {
    ...
    let actual_binary_path = temp_dir
        .path()
        .join("MemoryAnalysis")
        .join("format_validation.memscope");
