...
use memscope_rs::core::{
    MemScopeError, MemoryOperation, SystemErrorType, ErrorSeverity,
    ErrorRecovery, DefaultErrorRecovery, RecoveryAction,
...
fn test_error_recovery_system() {
    ...
    if let Some(RecoveryAction::Retry { max_attempts, delay_ms }) = action {
...
fn test_backward_compatibility_conversion() {
    ...
    let test_cases = vec![
        (TrackingError::AllocationFailed("alloc failed".to_string()), "memory"),
        (TrackingError::DeallocationFailed("dealloc failed".to_string()), "memory"),
        (TrackingError::SerializationError("json error".to_string()), "system"),
        (TrackingError::AnalysisError("analysis failed".to_string()), "analysis"),
        (TrackingError::ExportError("export failed".to_string()), "export"),
        (TrackingError::ThreadSafetyError("thread error".to_string()), "system"),
        (TrackingError::ConfigurationError("config error".to_string()), "config"),
        (TrackingError::InternalError("internal error".to_string()), "internal"),
        (TrackingError::IoError("io error".to_string()), "system"),
        (TrackingError::BorrowCheckError("borrow error".to_string()), "analysis"),
        (TrackingError::LifetimeError("lifetime error".to_string()), "analysis"),
    ];
    ...
    for (old_error, expected_category) in test_cases {
        ...
        assert!(
            ...
            "Message preservation failed: '{}' vs '{}'", original_message, back_message
        );
    }
}
...
fn test_result_adaptation() {
    ...
    let error_result: TrackingResult<i32> = Err(TrackingError::AllocationFailed("test".to_string()));
...
fn test_standard_error_conversions() {
    ...
    assert!(memscope_error.user_message().contains("I/O operation failed"));
    ...
    assert_eq!(memscope_error.category(), "system");
    assert!(memscope_error.user_message().contains("JSON serialization failed"));
}
...
fn test_custom_error_recovery() {
    ...
    impl ErrorRecovery for CustomRecovery {
        fn can_recover(&self, error: &MemScopeError) -> bool {
            ...
            matches!(error, MemScopeError::Memory { operation: MemoryOperation::Allocation, .. })
        }
        
        fn get_recovery_action(&self, error: &MemScopeError) -> Option<RecoveryAction> {
            if self.can_recover(error) {
                Some(RecoveryAction::Retry { max_attempts: 5, delay_ms: 200 })
            } else {
    ...
    assert!(matches!(action, Some(RecoveryAction::Retry { max_attempts: 5, delay_ms: 200 })));
