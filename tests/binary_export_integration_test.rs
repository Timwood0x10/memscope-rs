...
fn test_binary_export_integration() {
    ...
    tracker
        .fast_track_allocation(0x1000, 1024, "buffer".to_string())
        .unwrap();
    tracker
        .fast_track_allocation(0x2000, 512, "data".to_string())
        .unwrap();
    ...
    println!(
        "Looking for MemoryAnalysis directory at: {:?}",
        memory_analysis_dir
    );
                        ...
                        for sub_entry in sub_entries {
                            if let Ok(sub_entry) = sub_entry {
                                    ...
                                    == Some(std::ffi::OsStr::new("memscope"))
                                {
                                    ...
                                    println!(
                                        "✅ Found .memscope file with {} bytes",
                                        metadata.len()
                                    );
