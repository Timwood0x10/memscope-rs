...
fn create_large_dataset(allocation_count: usize) -> TrackingResult<MemoryTracker> {
    ...
    for i in 0..allocation_count {
        ...
        if i % 1000 == 0 && i > 0 { // More frequent progress updates for better visibility
            ...
            println!(
                "  Progress: {}/{} ({:.0} allocs/sec)",
                i, allocation_count, rate
            );
        }
    }
...
mod tests {
    ...
    fn test_large_dataset_100k() {
        ...
        // Find the actual file that was created (it might have .memscope extension)
        let actual_binary_path = if binary_path.exists() {
            binary_path.clone()
        } else {
            // Try with .memscope extension
            let memscope_path = binary_path.with_extension("memscope");
            if memscope_path.exists() {
                memscope_path
            } else {
                // Try in MemoryAnalysis subdirectory
                let analysis_dir = temp_dir.path().join("MemoryAnalysis");
                let analysis_path = analysis_dir.join("large_100k.memscope");
                if analysis_path.exists() {
                    analysis_path
                } else {
                    binary_path // Use original path for error reporting
                }
            }
        };
        
        let file_size = std::fs::metadata(&actual_binary_path).unwrap().len() as usize;
        ...
        println!(
            "   Export: {:.2}s, Parse: {:.2}s, File: {:.2}MB",
    ...
    fn test_very_large_dataset_1m() {
        ...
        assert!(
            ...
            "Export should complete within 2 minutes"
        );
        ...
        parser
            ...
            .expect("Failed to parse very large binary file");
        let parse_duration = parse_start.elapsed();
    ...
    fn test_concurrent_operations() {
        ...
        let tracker =
            Arc::new(create_large_dataset(allocation_count).expect("Failed to create dataset"));
        ...
        for thread_id in 0..thread_count {
            ...
            let handle = thread::spawn(move || {
                for op_id in 0..operations_per_thread {
                    let binary_path =
                        temp_dir_path.join(format!("concurrent_{}_{}.bin", thread_id, op_id));
    ...
    fn test_memory_pressure() {
        ...
        let tracker = create_large_dataset(allocation_count).expect("Failed to create dataset");
        ...
        match export_result {
            Ok(_) => {
                ...
                assert!(
                    binary_path.exists(),
                    "Binary file should be created under memory pressure"
                );
                assert!(file_size > 0, "File should have content");

                // Test that we can still parse it
                let mut parser = BinaryParser::with_options(BinaryParserOptions {
            ...
            Err(e) => {
                println!(
                    "⚠️  Export failed under memory pressure (this may be expected): {}",
                    e
                );
    ...
    fn test_long_running_operations() {
        ...
        let handle = thread::spawn(move || {
            ...
            while start_time.elapsed() < test_duration {
                ...
                let binary_path =
                    temp_dir_path.join(format!("long_running_{}.bin", operation_count));
                let options = BinaryExportOptions::default().compression(CompressionType::Lz4);
                ...
                match tracker.export_to_binary_with_options(&binary_path, options) {
                    Ok(_) => {
                        ...
                        if operation_count > 5 {
                            let old_path = temp_dir_path
                                .join(format!("long_running_{}.bin", operation_count - 5));
        ...
        assert!(
            total_operations >= 8,
            "Should complete at least 8 operations in 1 minute"
        );
        assert!(
            result.success_rate >= 90.0,
            "Success rate should be at least 90% for long running test"
        );
    ...
    fn test_error_recovery() {
        ...
        let tracker = create_large_dataset(25_000).expect("Failed to create dataset");

        // Create a binary file
        let binary_path = temp_dir.path().join("error_recovery.bin");
        tracker
            .export_to_binary(&binary_path)
            .expect("Initial export failed");
        ...
        match parse_result {
                ...
                match recovery_parser.load_allocations() {
                    Ok(allocations) => {
                        ...
                        assert!(
                            ...
                            "Should recover a significant portion of data"
                        );
                    }
                    Err(e) => {
            ...
            Err(e) => {
                println!(
                    "⚠️  Recovery parser failed (this may be expected for severe corruption): {}",
                    e
                );
            }
        }
        ...
        assert!(
            ...
            "Strict parser should reject corrupted file"
        );
    ...
    fn test_resource_cleanup() {
        ...
        for i in 0..20 {
            let tracker = create_large_dataset(5_000).expect("Failed to create dataset");
            ...
            parser
                .load_from_file(&binary_path)
                .expect("Parse failed during cleanup test");
        ...
        assert!(
            ...
            "Cleanup test should complete within 1 minute"
        );
    ...
    mod extreme_tests {
        ...
        fn test_extreme_large_dataset_10m() {
            ...
            let tracker =
                create_large_dataset(allocation_count).expect("Failed to create extreme dataset");
            ...
            assert!(
                export_duration.as_secs() < 600,
                "Even extreme dataset should export within 10 minutes"
            );
            assert!(
                file_size > 100 * 1024 * 1024,
                "File should be at least 100MB for 10M allocations"
            );
