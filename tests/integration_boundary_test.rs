...
fn test_mixed_allocation_patterns() {
    ...
    assert!(stats.active_allocations >= 5, "Should track multiple allocations");
    
    // Test export with mixed data
    let export_result = tracker.export_to_json("mixed_patterns.json");
    assert!(export_result.is_ok(), "Should export mixed allocation patterns");
...
fn test_rapid_allocation_deallocation() {
    ...
    assert!(final_stats.total_allocations > initial_stats.total_allocations + 50,
...
fn test_nested_scope_tracking() {
    ...
    // Test nested scopes
    let outer_scope = scope_tracker.enter_scope("outer_scope".to_string())
    ...
    {
        let inner_scope = scope_tracker.enter_scope("inner_scope".to_string())
        ...
        {
            let deep_scope = scope_tracker.enter_scope("deep_scope".to_string())
                .expect("Should enter deep scope");
            
            let deep_data = vec![7, 8, 9];
            let _ = track_var!(deep_data);
            
            scope_tracker.exit_scope(deep_scope).expect("Should exit deep scope");
        }
        
        scope_tracker.exit_scope(inner_scope).expect("Should exit inner scope");
    }
    
    scope_tracker.exit_scope(outer_scope).expect("Should exit outer scope");
    
    // Verify scope analysis
    let scope_analysis = scope_tracker.get_scope_analysis()
        .expect("Should get scope analysis");
    
    assert!(scope_analysis.total_scopes >= 3, "Should track nested scopes");
}
...
fn test_concurrent_tracking_with_shared_data() {
    ...
    assert_eq!(final_shared.len(), 50, "Should have 50 entries from all threads");
    
    // Verify tracking stats
    let stats = tracker.get_stats().expect("Should get stats");
    assert!(stats.total_allocations >= 50, "Should track concurrent allocations");
}
...
fn test_memory_pressure_scenarios() {
    ...
    assert!(pressure_stats.total_allocations >= initial_stats.total_allocations,
...
fn test_error_recovery_scenarios() {
    ...
    assert!(stats_result.is_ok(), "Stats retrieval should work after errors");
}
...
fn test_comprehensive_export_integration() {
    ...
    let scope_id = scope_tracker.enter_scope("integration_test".to_string())
    ...
    assert!(std::path::Path::new("comprehensive_test.json").exists(), "JSON file should exist");
    assert!(std::path::Path::new("comprehensive_test.svg").exists(), "SVG file should exist");
    assert!(std::path::Path::new("comprehensive_timeline.svg").exists(), "Timeline file should exist");
    assert!(std::path::Path::new("comprehensive_dashboard.html").exists(), "Dashboard file should exist");
    
    // Exit scope
    scope_tracker.exit_scope(scope_id).expect("Should exit scope");
