...
impl ApiCompatibilityChecker {
    ...
    pub fn run_all_tests(&mut self) -> bool {
        ...
        println!(
            "📊 API Compatibility Results: {}/{} tests passed",
    ...
    fn test_analysis_apis(&mut self) {
        ...
        self.run_test("perform_comprehensive_analysis", || {
            ...
            let _result =
                memscope_rs::analysis::perform_comprehensive_analysis(&allocations, &stats);
            Ok(())
        });
    ...
    fn test_utility_functions(&mut self) {
        ...
        self.run_test("simplify_type_name", || {
            let simplified =
                simplify_type_name("std::collections::HashMap<std::string::String, i32>");
        ...
        F: FnOnce() -> Result<(), Box<dyn std::error::Error>>,
    {
        ...
        if result.test_passed {
            println!(
                "   ✅ {} ({} ms)",
                result.api_name, result.execution_time_ms
            );
        } else {
            println!(
                "   ❌ {} ({} ms): {}",
    ...
    pub fn generate_report(&self) -> String {
        ...
        for result in &self.test_results {
            let status = if result.test_passed {
                "✅ PASS"
            } else {
                "❌ FAIL"
            };
            report.push_str(&format!(
                "- {} {} ({} ms)",
                status, result.api_name, result.execution_time_ms
            ));
