...
fn test_end_to_end_comprehensive_metrics() {
    ...
    println!(
        "✅ End-to-end test passed with {:.1}% metrics completeness",
        completeness_score * 100.0
    );
}
...
fn test_all_configuration_combinations() {
    ...
    for (config_name, config) in configs {
        ...
        let result = memscope_rs::export::binary::export_to_binary_with_config(
            &test_data, file_path, &config,
        );
        assert!(
            result.is_ok(),
            "Export failed for config {}: {:?}",
            config_name,
            result.err()
        );
        ...
        assert_eq!(
            read_data.len(),
            test_data.len(),
            "Data count mismatch for config {}",
            config_name
        );
...
fn test_multithreaded_consistency() {
    ...
    println!(
        "Actual stats.active_allocations: {}",
    ...
    assert!(
        stats.total_allocations >= min_expected,
    ...
    assert!(
        stats.total_deallocations >= min_expected_deallocations,
        "Should have tracked at least 95% of deallocations. Expected: {}, Got: {}",
        min_expected_deallocations,
    ...
    let expected_active_range = if std::env::var("MEMSCOPE_ACCURATE_TRACKING").is_ok() {
        ...
        let min_active = stats
            .total_allocations
            .saturating_sub(stats.total_deallocations + 50);
        let max_active = stats.total_allocations + 50;
        (min_active, max_active)
    };

    assert!(
        stats.active_allocations >= expected_active_range.0
            && stats.active_allocations <= expected_active_range.1,
        "Active allocations should be in range [{}, {}]. Got: {}",
        expected_active_range.0,
        expected_active_range.1,
        stats.active_allocations
    );
    ...
    println!(
        "✅ Multi-threaded consistency test passed with {}",
        tracking_mode
    );
    println!(
        "   Total allocations: {} ({}% of expected)",
        stats.total_allocations,
...
fn test_advanced_metrics_completeness() {
    ...
    for (i, (type_name, size)) in scenarios.iter().enumerate() {
        ...
        tracker
            .associate_var(ptr, format!("test_var_{i}"), type_name.to_string())
            .unwrap();
    ...
    println!(
        "   Basic fields completeness: {:.1}%",
        completeness_result.basic_fields_completeness * 100.0
    );
    println!(
        "   Advanced fields completeness: {:.1}%",
        completeness_result.advanced_fields_completeness * 100.0
    );
    println!(
        "   Lifecycle data completeness: {:.1}%",
        completeness_result.lifecycle_completeness * 100.0
    );
    println!(
        "   Container analysis completeness: {:.1}%",
        completeness_result.container_analysis_completeness * 100.0
    );
    println!(
        "   Type analysis completeness: {:.1}%",
        completeness_result.type_analysis_completeness * 100.0
    );

    // Task 14 requirement: Verify 95%+ completeness
    assert!(
        completeness_result.overall_completeness >= 0.95,
    ...
    assert!(
        completeness_result.basic_fields_completeness >= 0.98,
        "Basic fields should be nearly 100% complete, got {:.2}%",
        completeness_result.basic_fields_completeness * 100.0
    );

    assert!(
        completeness_result.advanced_fields_completeness >= 0.90,
    ...
    if metrics_coverage.smart_pointer_expected > 0 {
        assert!(
            metrics_coverage.smart_pointer_coverage >= 0.50,
    ...
    assert!(
        metrics_coverage.container_coverage >= 0.95,
        "Container analysis coverage should be >= 95%, got {:.2}%",
        metrics_coverage.container_coverage * 100.0
    );

    assert!(
        metrics_coverage.lifecycle_coverage >= 0.95,
...
fn test_binary_format_roundtrip_comprehensive() {
    ...
    memscope_rs::export::binary::export_to_binary_with_config(&original_data, file_path, &config)
        .unwrap();
...
fn test_performance_under_load() {
    ...
    for i in 0..num_allocations {
        ...
        if i % 100 == 0 {
            tracker
                .associate_var(ptr, format!("bulk_var_{}", i), "BulkTestType".to_string())
                .unwrap();
        }
    ...
    assert!(
        allocation_time.as_millis() < 5000,
...
fn create_test_allocations(tracker: &MemoryTracker, count: usize) -> Vec<AllocationInfo> {
    for i in 0..count {
        ...
        tracker.track_allocation(ptr, size).unwrap();
        tracker
            .associate_var(ptr, format!("test_var_{}", i), format!("TestType{}", i % 5))
            .unwrap();
    }
...
fn is_smart_pointer_type(type_name: &str) -> bool {
    type_name.starts_with("Rc<") || type_name.starts_with("Arc<") || type_name.starts_with("Box<")
}
