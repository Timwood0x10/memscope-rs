...
fn test_ffi_boundary_detection() {
    ...
    let stats = unsafe_tracker.get_stats();
    println!(
        "FFI boundary detection stats: {} total operations, {} FFI calls",
    ...
    if stats.ffi_calls > 0 {
        ...
        println!(
            "Operation tracked as general operation: {}",
...
fn test_enhanced_deallocation() {
    ...
    unsafe_tracker
        .track_unsafe_allocation(0x1000, 100, "test_allocation".to_string())
...
fn test_boundary_event_recording() {
    ...
    // First allocate something
    unsafe_tracker
        .track_unsafe_allocation(0x2000, 256, "boundary_test".to_string())
        .expect("Should track allocation");
...
fn test_leak_detection() {
    ...
    unsafe_tracker
        .track_unsafe_allocation(0x3000, 512, "potential_leak_test".to_string())
        .expect("Should track allocation");
...
fn test_unsafe_ffi_statistics() {
    ...
    unsafe_tracker
        .track_unsafe_allocation(0x4000, 128, "test_unsafe_block".to_string())
        .expect("Should track unsafe allocation");
...
fn test_unsafe_ffi_export_integration() {
    ...
    unsafe_tracker
        .track_unsafe_allocation(0x6000, 64, "export_test".to_string())
        .expect("Should track unsafe allocation");
    ...
    if let Ok(content) = std::fs::read_to_string("unsafe_ffi_test.json") {
        assert!(
            content.contains("unsafe_ffi_analysis"),
            "Should contain unsafe/FFI analysis"
        );
    }
