...
fn test_container_analysis_different_types() {
    ...
    for (type_name, size, expected_container_type) in test_cases {
        ...
        assert!(
            layout.container_analysis.is_some(),
            "Container analysis missing for {}",
            type_name
        );
...
fn test_non_container_types_no_analysis() {
    ...
    let non_container_types = vec![("i32", 4), ("f64", 8), ("bool", 1), ("CustomStruct", 32)];
    ...
    for (type_name, size) in non_container_types {
        ...
        assert!(
            layout.container_analysis.is_none(),
            "Container analysis should be None for non-container type: {}",
            type_name
        );
    }
}
...
fn test_capacity_utilization_efficiency_assessment() {
    ...
    for (size, description) in test_cases {
        ...
        assert!(
            utilization.utilization_ratio >= 0.0,
            "Invalid utilization ratio for {}: {}",
            description,
            utilization.utilization_ratio
        );
        assert!(
            utilization.utilization_ratio <= 1.0,
            "Invalid utilization ratio for {}: {}",
            description,
            utilization.utilization_ratio
        );
        ...
        if utilization.current_capacity > utilization.current_length {
            assert!(
                utilization.wasted_space > 0,
                "Expected wasted space for {}",
                description
            );
        }
    }
...
fn test_reallocation_pattern_optimization_suggestions() {
    ...
    assert!(
        !patterns.optimization_suggestions.is_empty(),
    ...
    assert!(
        suggestions_text.contains("with_capacity")
