...
fn test_all_tracker_methods() {
    ...
    assert!(
        stats.total_allocations < 1_000_000,
    ...
    let active = tracker
        ...
        .expect("Should get active allocations");
    ...
    let by_type = tracker
        .get_memory_by_type()
        .expect("Should get memory by type");
...
fn test_all_scope_tracker_methods() {
    ...
    let scope1 = scope_tracker
        .enter_scope("test_scope_1".to_string())
        .expect("Should enter scope 1");

    let scope2 = scope_tracker
        .enter_scope("test_scope_2".to_string())
    ...
    scope_tracker
        .exit_scope(scope2)
        .expect("Should exit scope 2");
    scope_tracker
        .exit_scope(scope1)
        .expect("Should exit scope 1");
...
fn test_all_unsafe_ffi_methods() {
    ...
    let ptr_result =
        unsafe_tracker.track_unsafe_allocation(0x2000, 100, "test_pointer_op".to_string());
    ...
    println!(
        "Unsafe/FFI tracker stats: {} operations, risk score: {}",
...
fn test_data_structure_coverage() {
    ...
    let stats = tracker.get_stats().expect("Should get stats");
    println!(
        "Data structure coverage: {} allocations tracked",
