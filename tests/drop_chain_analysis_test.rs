...
fn test_drop_chain_analysis_smart_pointers() {
    ...
    assert_eq!(analysis.root_object.drop_impl_type, 
...
fn test_drop_chain_analysis_resource_handles() {
    ...
    let has_file_cleanup = analysis.root_object.cleanup_actions.iter()
        .any(|action| matches!(action.action_type, 
...
fn test_drop_chain_performance_analysis() {
    ...
    let types_to_test = vec![
        ...
        ("std::fs::File", memscope_rs::core::types::ImpactLevel::Medium),
        ("std::net::TcpStream", memscope_rs::core::types::ImpactLevel::Medium),
    ];
    ...
    for (type_name, expected_min_impact) in types_to_test {
        ...
        assert!(analysis.root_object.performance_characteristics.execution_time_ns > 0);
        assert!(analysis.root_object.performance_characteristics.cpu_usage_percent >= 0.0);
        
        // Verify impact level is reasonable (just check it's a valid enum value)
        let _impact = &analysis.root_object.performance_characteristics.impact_level;
...
fn test_drop_chain_leak_detection() {
    ...
    assert!(!analysis.leak_detection.prevention_recommendations.is_empty());
    
    // Should recommend weak references for Rc types
    let has_weak_ref_recommendation = analysis.leak_detection.prevention_recommendations.iter()
        .any(|rec| matches!(rec.recommendation_type, 
...
fn test_drop_chain_integration_with_deallocation() {
    ...
    tracker.associate_var(0x9000, "test_vec".to_string(), "Vec<String>".to_string()).unwrap();
    ...
    let deallocated = stats.allocations.iter()
        .find(|alloc| alloc.ptr == 0x9000);
